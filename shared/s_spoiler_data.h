...
typedef enum SpoilerCollectionCheckType {
    SPOILER_CHK_NONE,
...
typedef enum SpoilerCollectionCheckGroup {
    GROUP_NO_GROUP,
    GROUP_KOKIRI_FOREST,              // 0x55, 0x28
    GROUP_LOST_WOODS,                 // 0x5B
    GROUP_SACRED_FOREST_MEADOW,       // 0x56
    GROUP_DUNGEON_DEKU_TREE,          // 0x00, 0x11
    GROUP_DUNGEON_FOREST_TEMPLE,      // 0x03
    GROUP_KAKARIKO,                   // 0x37, 0x42, 0x48, 0x52
    GROUP_GRAVEYARD,                  // 0x3F, 0x40, 0x41, 0x48, 0x53
    GROUP_DUNGEON_BOTTOM_OF_THE_WELL, // 0x08
    GROUP_DUNGEON_SHADOW_TEMPLE,      // 0x07
    GROUP_DEATH_MOUNTAIN_TRAIL,       // 0x60
    GROUP_DEATH_MOUNTAIN_CRATER,      // 0x61
    GROUP_GORON_CITY,                 // 0x62
    GROUP_DUNGEON_DODONGOS_CAVERN,    // 0x01, 0x12
    GROUP_DUNGEON_FIRE_TEMPLE,        // 0x04
    GROUP_ZORAS_RIVER,                // 0x54
    GROUP_ZORAS_DOMAIN,               // 0x58
    GROUP_ZORAS_FOUNTAIN,             // 0x59
    ...
    GROUP_GERUDO_VALLEY,              // 0x5A, 0x5D, 0x0C
    GROUP_HAUNTED_WASTELAND,          // 0x5E, 0x5C
    GROUP_GERUDO_TRAINING_GROUND,     // 0x0B
    GROUP_DUNGEON_SPIRIT_TEMPLE,      // 0x06
    GROUP_MARKET,                     // 0x10, 0x4B, 0x35, 0x42, 0x4D
    GROUP_HYRULE_CASTLE,              // 0x5F, 0x4A
...
typedef enum SpoilerItemCollectType {
    COLLECTTYPE_NORMAL,
    COLLECTTYPE_REPEATABLE,
    COLLECTTYPE_NEVER,
} SpoilerItemCollectType;

typedef enum SpoilerItemRevealType {
    REVEALTYPE_NORMAL,
    REVEALTYPE_SCENE,
    REVEALTYPE_ALWAYS,
} SpoilerItemRevealType;

typedef struct SpoilerItemLocation {
...
typedef struct SpoilerSphere {
    u8 ItemCount;
    u16 ItemLocationsOffset;
} SpoilerSphere;

typedef struct SpoilerData {
    u8 SphereCount;
...
typedef struct SpoilerDataLocs {
