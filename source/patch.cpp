...
bool WriteAllPatches() {
    ...
    patchOffset                                              = V_TO_P(gPatchSymbols.rEntranceOverrides);
    ...
    patchOffset = V_TO_P(gPatchSymbols.gSettingsContext);
    ...
    patchOffset = V_TO_P(gPatchSymbols.gSpoilerData);
    ...
    for (size_t idx = 0; idx < SPOILER_LOCDATS; idx++) {
        patchOffset = V_TO_P(gPatchSymbols.gSpoilerDataLocs) + (patchSize * idx);
    ...
    if (ctx.scrubsanity == SCRUBSANITY_RANDOM_PRICES) {
        ...
        patchOffset = V_TO_P(gPatchSymbols.rScrubRandomItemPrices);
    ...
    if (Settings::Shopsanity.IsNot(SHOPSANITY_OFF) && Settings::Shopsanity.IsNot(SHOPSANITY_ZERO)) {
        ...
        patchOffset = V_TO_P(gPatchSymbols.rShopsanityPrices);
    ...
    if (Settings::ShuffleRewards.Is(REWARDSHUFFLE_END_OF_DUNGEON)) {
        patchOffset = V_TO_P(gPatchSymbols.rDungeonRewardOverrides);
    ...
    u32 messageDataOffset = V_TO_P(gPatchSymbols.rCustomMessages);
    ...
    patchOffset                     = V_TO_P(gPatchSymbols.ptrCustomMessageEntries);
    patchSize                       = 4;
    ...
    patchOffset                     = V_TO_P(gPatchSymbols.numCustomMessageEntries);
    patchSize                       = 4;
    ...
    if (!Music::archiveFound) {
        patchOffset = V_TO_P(gPatchSymbols.rBGMOverrides);
    ...
    patchOffset = V_TO_P(gPatchSymbols.rSfxData);
    ...
    */
    ...
    patchOffset = V_TO_P(gPatchSymbols.rGsLocOverrides);
    ...
    if (Settings::Enemizer) {
        ...
        patchOffset = V_TO_P(gPatchSymbols.rEnemyOverrides);
