...
void HintTable_Init_Exclude_Overworld() {
    hintTable[KF_KOKIRI_SWORD_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"the #hidden treasure of the Kokiri# is", /*french*/"le #trésor des Kokiri# est", /*spanish*/"el #tesoro oculto de los Kokiri# esconde", /*italian*/"il #tesoro nascosto dei Kokiri# è", /*german*/"Deutsch"},
  });

    hintTable[KF_MIDOS_TOP_LEFT_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"the #leader of the Kokiri# hides", /*french*/"le #chef des Kokiri# possède", /*spanish*/"el #líder de los Kokiri# esconde", /*italian*/"il #capo dei Kokiri# nasconde", /*german*/"Deutsch"},
                     }, {},
                       //clear text
                       Text{"#inside Mido's house# is", /*french*/"#dans la maison de Mido# gît", /*spanish*/"en la #casa de Mido# yace", /*italian*/"nella #casa di Mido# puoi trovare", /*german*/"Deutsch"}
  );
    ...
    hintTable[GRAVEYARD_SHIELD_GRAVE_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"the #treasure of a fallen soldier# is", /*french*/"le #trésor du soldat mort# est", /*spanish*/"el #tesoro de un soldado caído# esconde", /*italian*/"il #tesoro di un soldato caduto# è", /*german*/"Deutsch"},
  });

    hintTable[DMT_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"hidden behind a wall on a #mountain trail# is", /*french*/"derrière une façade du #chemin montagneux# est", /*spanish*/"tras una pared del #sendero de la montaña# yace", /*italian*/"dietro una parete su un #sentiero di montagna# puoi trovare", /*german*/"Deutsch"},
  });

    hintTable[GC_MAZE_RIGHT_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"in #Goron City# explosives unlock", /*french*/"des explosions dans le #village Goron# révèlent", /*spanish*/"en la #Ciudad Goron# unos explosivos desbloquean", /*italian*/"nella #città dei Goron# gli esplosivi sbloccano", /*german*/"Deutsch"},
  });
    ...
    hintTable[ZD_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"fire #beyond a waterfall# reveals", /*french*/"du feu #derrière la cascade# éclaire", /*spanish*/"las #llamas tras una una cascada# revelan", /*italian*/"#fuoco oltre una cascata# rivela", /*german*/"Deutsch"},
  });

    hintTable[GRAVEYARD_HOOKSHOT_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"a chest hidden by a #speedy spectre# holds", /*french*/"le #coffre du rapide revenant# contient", /*spanish*/"un cofre custodiado por un #espectro veloz# contiene", /*italian*/"un forziere nascosto da uno #svelto spettro# contiene", /*german*/"Deutsch"},
                     }, {},
                       //clear text
                       Text{"#dead Dampé's first prize# is", /*french*/"la #première course d'Igor# donne", /*spanish*/"el primer premio de #la carrera de Dampé# se trata de", /*italian*/"la prima ricompensa del #fantasma di Danpei# è", /*german*/"Deutsch"}
  );

    hintTable[GF_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"on a #rooftop in the desert# lies", /*french*/"sur un #toit du désert# gît", /*spanish*/"en una #azotea del desierto# yace", /*italian*/"sopra un #tetto in un deserto# puoi trovare", /*german*/"Deutsch"},
  });

    hintTable[KAK_REDEAD_GROTTO_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"#zombies beneath the earth# guard", /*french*/"les #revenants sous terre# protègent", /*spanish*/"unos #zombis subterráneos# esconden", /*italian*/"dei #ri-morti sottoterra# proteggono", /*german*/"Deutsch"},
  });

    hintTable[SFM_WOLFOS_GROTTO_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"#wolves beneath the earth# guard", /*french*/"les #loups sous terre# protègent", /*spanish*/"unos #lobos subterráneos# esconden", /*italian*/"dei #lupi sottoterra# proteggono", /*german*/"Deutsch"},
  });

    hintTable[HF_NEAR_MARKET_GROTTO_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"a #hole in a field near a drawbridge# holds", /*french*/"la #grotte près d'un pont# contient", /*spanish*/"bajo el #hoyo de una llanura cercano a un puente# yace", /*italian*/"#vicino al ponte levatoio# qualcuno ha sepolto", /*german*/"Deutsch"},
  });

    hintTable[HF_SOUTHEAST_GROTTO_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"a #hole amongst trees in a field# holds", /*french*/"la #grotte près des arbres# contient", /*spanish*/"bajo el #hoyo de una llanura rodeado de árboles# yace", /*italian*/"#in mezzo a degli alberi# in una pianura qualcuno ha sepolto", /*german*/"Deutsch"},
  });

    hintTable[HF_OPEN_GROTTO_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"an #open hole in a field# holds", /*french*/"la #grotte dans les plaines# contient", /*spanish*/"bajo el #hoyo descubierto de una llanura# yace", /*italian*/"una #buca aperta nella pianura# contiene", /*german*/"Deutsch"},
  });

    hintTable[KAK_OPEN_GROTTO_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"an #open hole in a town# holds", /*french*/"la #grotte dans le village# contient", /*spanish*/"bajo el #hoyo descubierto de un pueblo# yace", /*italian*/"una #buca aperta in un villaggio# contiene", /*german*/"Deutsch"},
  });

    hintTable[ZR_OPEN_GROTTO_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"a #hole along a river# holds", /*french*/"la #grotte près du fleuve# contient", /*spanish*/"bajo un #hoyo junto a un río# yace", /*italian*/"una #buca aperta in riva al fiume# contiene", /*german*/"Deutsch"},
  });

    hintTable[KF_STORMS_GROTTO_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"a #hole in a forest village# holds", /*french*/"la #grotte inondée de pluie dans le Village Kokiri# révèle", /*spanish*/"bajo el #hoyo de una tribu del bosque# yace", /*italian*/"nella #foresta dei Kokiri# qualcuno ha sepolto", /*german*/"Deutsch"},
  });

    hintTable[LW_NEAR_SHORTCUTS_GROTTO_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"a #hole in a wooded maze# holds", /*french*/"la #grotte dans le labyrinthe sylvestre# contient", /*spanish*/"bajo un #hoyo de un laberinto forestal# yace", /*italian*/"#in un dedalo silvestre# qualcuno ha sepolto", /*german*/"Deutsch"},
  });

    hintTable[DMT_STORMS_GROTTO_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"a #hole flooded with rain on a mountain# holds", /*french*/"la #grotte inondée de pluie sur la montagne# contient", /*spanish*/"bajo un #hoyo de una montaña inundado de lluvia# yace", /*italian*/"#all'ingresso di una città rocciosa# qualcuno ha sepolto", /*german*/"Deutsch"},
  });

    hintTable[DMC_UPPER_GROTTO_CHEST] = HintText::Exclude({
                       //obscure text
                       Text{"a #hole in a volcano# holds", /*french*/"la #grotte dans le volcan# contient", /*spanish*/"bajo el #hoyo de un volcán# yace", /*italian*/"#in un vulcano# qualcuno ha sepolto", /*german*/"Deutsch"},
  });

    hintTable[TOT_MASTER_SWORD] = HintText::Exclude({
                       //obscure text
                       Text{"a #pedestal in a temple# holds", /*french*/"un #piédestal dans un temple# contient", /*spanish*/"un #pedestal en un templo# sostiene", /*italian*/"su #un piedistallo in un santuario# puoi ammirare", /*german*/"Deutsch"},
  });

    hintTable[TOT_LIGHT_ARROWS_CUTSCENE] = HintText::Exclude({
                       //obscure text
                       Text{"the #final gift of a princess# is", /*french*/"le #cadeau d'adieu de la princesse# est", /*spanish*/"el #obsequio final de la princesa# se trata de", /*italian*/"l'#ultimo dono della principessa# è", /*german*/"Deutsch"},
  });

    hintTable[LW_GIFT_FROM_SARIA] = HintText::Exclude({
                       //obscure text
                       Text{"a #potato hoarder# holds",           /*french*/"le #panini mélodieux# est en fait", /*spanish*/"cierta #jovencita verde# concede", /*italian*/"#chi esce dalla foresta proibita# riceve", /*german*/"Deutsch"},
                       Text{"a rooty tooty #flutey cutey# gifts", /*french*/"la #patate musicale# est en fait",  /*spanish*/"una #gran amiga# concede",         /*italian*/"una #piccola pifferaia# dona",             /*german*/"Deutsch"},
                     }, {},
                       //clear text
                       Text{"#Saria's Gift# is", /*french*/"le #cadeau de Saria# est", /*spanish*/"el #regalo de Saria# se trata de", /*italian*/"il #regalo di Saria# è", /*german*/"Deutsch"}
  );

    hintTable[ZF_GREAT_FAIRY_REWARD] = HintText::Exclude({
                       //obscure text
                       Text{"the #fairy of winds# holds", /*french*/"la #fée du vent# a", /*spanish*/"el #hada del viento# brinda", /*italian*/"la #fata del vento# consegna", /*german*/"Deutsch"},
  });

    hintTable[HC_GREAT_FAIRY_REWARD] = HintText::Exclude({
                       //obscure text
                       Text{"the #fairy of fire# holds", /*french*/"la #fée du feu# a", /*spanish*/"el #hada del fuego# brinda", /*italian*/"la #fata del fuoco# consegna", /*german*/"Deutsch"},
  });

    hintTable[COLOSSUS_GREAT_FAIRY_REWARD] = HintText::Exclude({
                       //obscure text
                       Text{"the #fairy of love# holds", /*french*/"la #fée de l'amour# a", /*spanish*/"el #hada del amor# brinda", /*italian*/"la #fata dell'amore# consegna", /*german*/"Deutsch"},
  });
    ...
    hintTable[SONG_FROM_IMPA] = HintText::Exclude({
                       //obscure text
                       Text{"#deep in a castle#, Impa teaches", /*french*/"#la gardienne de la princesse# donne", /*spanish*/"en el #jardín del castillo Impa enseña#", /*italian*/"#tra le mura del castello#, Impa insegna", /*german*/"Deutsch"},
  });

    hintTable[SONG_FROM_MALON] = HintText::Exclude({
                       //obscure text
                       Text{"#a farm girl# sings", /*french*/"la #fillette de la ferme# donne", /*spanish*/"una #chica rupestre# canta", /*italian*/"una #ragazza di campagna# canta", /*german*/"Deutsch"},
  });

    hintTable[SONG_FROM_SARIA] = HintText::Exclude({
                       //obscure text
                       Text{"#deep in the forest#, Saria teaches", /*french*/"la #fille de la forêt# donne", /*spanish*/"al #fondo del bosque# Saria enseña", /*italian*/"#in fondo al bosco#, Saria insegna", /*german*/"Deutsch"},
  });

    hintTable[SONG_FROM_WINDMILL] = HintText::Exclude({
                       //obscure text
                       Text{"a man #in a windmill# is obsessed with", /*french*/"l'#homme du moulin# donne", /*spanish*/"el #hombre del molino# está obsesionado con", /*italian*/"#l'uomo del mulino# ha un'ossessione per", /*german*/"Deutsch"},
  });


    hintTable[HC_MALON_EGG] = HintText::Exclude({
                       //obscure text
                       Text{"a #girl looking for her father# gives", /*french*/"la #fillette qui cherche son père# donne", /*spanish*/"una #chica en busca de su padre# otorga", /*italian*/"una #ragazza in cerca di suo padre# dona", /*german*/"Deutsch"},
  });

    hintTable[HC_ZELDAS_LETTER] = HintText::Exclude({
                       //obscure text
                       Text{"a #princess in a castle# gifts", /*french*/"la #princesse dans le château# donne", /*spanish*/"la #princesa de un castillo# otorga", /*italian*/"la #principessa nel castello# consegna", /*german*/"Deutsch"},
  });

    hintTable[ZD_DIVING_MINIGAME] = HintText::Exclude({
                       //obscure text
                       Text{"an #unsustainable business model# gifts", /*french*/"le #mauvais modèle d'affaires# donne", /*spanish*/"un #mal modelo de negocio# premia con", /*italian*/"una #pessima strategia commerciale# elargisce", /*german*/"Deutsch"},
                     }, {},
                       //clear text
                       Text{"those who #dive for Zora rupees# will find", /*french*/"ceux qui #plongent pour des rubis Zora# trouveront", /*spanish*/"aquellos que se #sumergan por las rupias zora# encontrarán", /*italian*/"chi #si immerge per recuperare rupie fra gli Zora# riceve", /*german*/"Deutsch"}
  );

    hintTable[LH_CHILD_FISHING] = HintText::Exclude({
                       //obscure text
                       Text{"#fishing in youth# bestows", /*french*/"#pêcher dans sa jeunesse# promet", /*spanish*/"#pescar en la juventud# conduce a", /*italian*/"#pescare in gioventù# procura", /*german*/"Deutsch"},
  });

    hintTable[LH_ADULT_FISHING] = HintText::Exclude({
                       //obscure text
                       Text{"#fishing in maturity# bestows", /*french*/"#pêcher dans sa maturité# promet", /*spanish*/"#pescar en la madurez# conduce a", /*italian*/"#pescare dopo la maturità# procura", /*german*/"Deutsch"},
  });

    hintTable[LH_LAB_DIVE] = HintText::Exclude({
                       //obscure text
                       Text{"a #diving experiment# is rewarded with", /*french*/"l'#expérience de plongée# donne", /*spanish*/"#bucear para un experimento# se premia con", /*italian*/"una #prova di immersione# è ricompensata con", /*german*/"Deutsch"},
  });

    hintTable[GC_ROLLING_GORON_AS_ADULT] = HintText::Exclude({
                       //obscure text
                       Text{"#comforting yourself# provides", /*french*/"se #réconforter soi-même# donne", /*spanish*/"#confrontarte a ti mismo# otorga", /*italian*/"#confortare se stessi# procura", /*german*/"Deutsch"},
                     }, {},
                       //clear text
                       Text{"#reassuring a young Goron# is rewarded with", /*french*/"#rassurer un jeune Goron# donne", /*spanish*/"#calmar a un joven Goron# otorga", /*italian*/"#calmare un giovane Goron# procura", /*german*/"Deutsch"}
  );

    hintTable[MARKET_BOMBCHU_BOWLING_FIRST_PRIZE] = HintText::Exclude({
                       //obscure text
                       Text{"the #first explosive prize# is", /*french*/"le #premier prix explosif# est", /*spanish*/"el #primer premio explosivo# se trata de", /*italian*/"il #primo premio esplosivo# è", /*german*/"Deutsch"},
  });

    hintTable[MARKET_BOMBCHU_BOWLING_SECOND_PRIZE] = HintText::Exclude({
                       //obscure text
                       Text{"the #second explosive prize# is", /*french*/"le #deuxième prix explosif# est", /*spanish*/"el #segundo premio explosivo# se trata de", /*italian*/"il #secondo premio esplosivo# è", /*german*/"Deutsch"},
  });

    hintTable[MARKET_LOST_DOG] = HintText::Exclude({
                       //obscure text
                       Text{"#puppy lovers# will find", /*french*/"les #amoureux canins# trouveront", /*spanish*/"los #amantes caninos# encontrarán", /*italian*/"i #cinofili# riceveranno", /*german*/"Deutsch"},
                     }, {},
                       //clear text
                       Text{"#rescuing Richard the Dog# is rewarded with", /*french*/"#retrouver Kiki le chien# promet", /*spanish*/"#rescatar al perrito Ricardo# conduce a", /*italian*/"#salvare Richard il cane# viene ricompensato con", /*german*/"Deutsch"}
  );

    hintTable[LW_OCARINA_MEMORY_GAME] = HintText::Exclude({
                       //obscure text
                       Text{"the prize for a #game of Simon Says# is", /*french*/"la #récompense de Jean Dit# est", /*spanish*/"#repetir ciertas melodías# otorga", /*italian*/"serve #buona memoria# per vincere", /*german*/"Deutsch"},
                       Text{"a #child sing-a-long# holds",             /*french*/"les #jeunes flûtistes# donnent",  /*spanish*/"#tocar junto a otros# otorga",      /*italian*/"#imitare melodie# procura",         /*german*/"Deutsch"},
                     }, {},
                       //clear text
                       Text{"#playing an Ocarina in Lost Woods# is rewarded with", /*french*/"#jouer l'ocarina dans la forêt des méandres# donne", /*spanish*/"#tocar la ocarina en el Bosque Perdido# otorga", /*italian*/"#suonare l'ocarina nel bosco perduto# procura", /*german*/"Deutsch"}
  );

    hintTable[KAK_10_GOLD_SKULLTULA_REWARD] = HintText::Exclude({
                       //obscure text
                       Text{"#10 bug badges# rewards",           /*french*/"#10 écussons# donnent",             /*spanish*/"#10 medallas de insectos# otorgan", /*italian*/"#10 spille d'insetto# sono premiate con", /*german*/"Deutsch"},
                       Text{"#10 spider souls# yields",          /*french*/"#10 âmes# donnent",                 /*spanish*/"#10 almas de araña# otorgan",       /*italian*/"#10 anime di ragno# forniscono",          /*german*/"Deutsch"},
                       Text{"#10 auriferous arachnids# lead to", /*french*/"#10 arachnides aurifères# donnent", /*spanish*/"#10 arácnidos auríferos# otorgan",  /*italian*/"#10 artropodi aurei# forniscono",         /*german*/"Deutsch"},
                     }, {},
                       //clear text
                       Text{"slaying #10 Gold Skulltulas# reveals", /*french*/"détruire #10 Skulltulas d'or# donne", /*spanish*/"#exterminar 10 skulltulas doradas# revela", /*italian*/"eliminare #10 Aracnule d'oro# rivela", /*german*/"Deutsch"}
  );

    hintTable[KAK_MAN_ON_ROOF] = HintText::Exclude({
                       //obscure text
                       Text{"a #rooftop wanderer# holds", /*french*/"une #rencontre sur un toit# donne", /*spanish*/"#alguien sobre un tejado# otorga", /*italian*/"#un tizio su un tetto# possiede", /*german*/"Deutsch"},
  });

    hintTable[ZR_MAGIC_BEAN_SALESMAN] = HintText::Exclude({
                       //obscure text
                       Text{"a seller of #colorful crops# has", /*french*/"le #marchand de légumes# vend", /*spanish*/"el vendedor de un #colorido cultivo# ofrece", /*italian*/"un #mercante di ortaggi colorati# vende", /*german*/"Deutsch"},
                     }, {},
                       //clear text
                       Text{"a #bean seller# offers", /*french*/"le #marchand de haricots magiques# vend en fait", /*spanish*/"el #vendedor de judías# ofrece", /*italian*/"il #venditore di fagioli# ha finito i fagioli, quindi ora vende", /*german*/"Deutsch"}
  );

    hintTable[ZR_FROGS_IN_THE_RAIN] = HintText::Exclude({
                       //obscure text
                       Text{"#frogs in a storm# gift", /*french*/"#les grenouilles mouillées# donnent", /*spanish*/"las #ranas bajo la tormenta# regalan", /*italian*/"delle #rane in una tempesta# regalano", /*german*/"Deutsch"},
  });
    ...
    hintTable[ZR_FROGS_ZELDAS_LULLABY] = HintText::Exclude({
                       ...
                       //clear text
                       Text{"after hearing #Zelda's Lullaby, the frogs# gift", /*french*/"à l'écoute de #la berceuse de Zelda, les grenouilles# donnent", /*spanish*/"después de escuchar #la Nana de Zelda, las ranas# regalan", /*italian*/"dopo aver ascoltato #la Ninna nanna di Zelda, le rane# regalano", /*german*/"Deutsch"}
  );
    ...
    hintTable[ZR_FROGS_EPONAS_SONG] = HintText::Exclude({
                       ...
                       //clear text
                       Text{"after hearing #Epona's Song, the frogs# gift", /*french*/"à l'écoute du #chant d'Epona, les grenouilles# donnent", /*spanish*/"después de escuchar #la Canción de Epona, las ranas# regalan", /*italian*/"dopo aver ascoltato #la Canzone di Epona, le rane# regalano", /*german*/"Deutsch"}
  );
    ...
    hintTable[ZR_FROGS_SARIAS_SONG] = HintText::Exclude({
                       ...
                       //clear text
                       Text{"after hearing #Saria's Song, the frogs# gift", /*french*/"à l'écoute du #chant de Saria, les grenouilles# donnent", /*spanish*/"después de escuchar #la Canción de Saria, las ranas# regalan", /*italian*/"dopo aver ascoltato #la Canzone di Saria, le rane# regalano", /*german*/"Deutsch"}
  );
    ...
    hintTable[ZR_FROGS_SUNS_SONG] = HintText::Exclude({
                       ...
                       //clear text
                       Text{"after hearing #the Sun's Song, the frogs# gift", /*french*/"à l'écoute du #chant du soleil, les grenouilles# donnent", /*spanish*/"después de escuchar #la Canción del Sol, las ranas# regalan", /*italian*/"dopo aver ascoltato #il Canto del sole, le rane# regalano", /*german*/"Deutsch"}
  );
    ...
    hintTable[GF_HBA_1000_POINTS] = HintText::Exclude({
                       //obscure text
                       Text{"scoring 1000 in #horseback archery# grants", /*french*/"obtenir 1000 points dans l'#archerie équestre# donne", /*spanish*/"conseguir 1000 puntos en el #tiro con arco a caballo# premia", /*italian*/"se totalizzi almeno 1000 punti al #tiro con l'arco a cavallo# riceverai", /*german*/"Deutsch"},
  });

    hintTable[MARKET_SHOOTING_GALLERY_REWARD] = HintText::Exclude({
                       //obscure text
                       Text{"#shooting in youth# grants", /*french*/"#faire du tir dans sa jeunesse# donne", /*spanish*/"#disparar en la juventud# otorga", /*italian*/"#sparare ai soldi# in gioventù procura", /*german*/"Deutsch"},
  });

    hintTable[KAK_SHOOTING_GALLERY_REWARD] = HintText::Exclude({
                       //obscure text
                       Text{"#shooting in maturity# grants", /*french*/"#faire du tir dans sa maturité# donne", /*spanish*/"#disparar en la madurez# otorga", /*italian*/"#sparare ai soldi# da adulti procura", /*german*/"Deutsch"},
  });

    hintTable[LW_TARGET_IN_WOODS] = HintText::Exclude({
                       //obscure text
                       Text{"shooting a #target in the woods# grants", /*french*/"#tirer une cible dans les bois# donne", /*spanish*/"disparar a un #blanco forestal# brinda", /*italian*/"#centrare un ceppo# ti fa sputare in faccia", /*german*/"Deutsch"},
  });

    hintTable[KAK_ANJU_AS_ADULT] = HintText::Exclude({
                       //obscure text
                       Text{"a #chicken caretaker# offers adults", /*french*/"devenir un #éleveur de Cocottes# donne", /*spanish*/"una #cuidadora de emplumados# le ofrece a los mayores", /*italian*/"un'#allevatrice di polli# affida agli adulti", /*german*/"Deutsch"},
  });

    hintTable[LLR_TALONS_CHICKENS] = HintText::Exclude({
                       //obscure text
                       Text{"#finding Super Cuccos# is rewarded with", /*french*/"#trouver des Super Cocottes# donne", /*spanish*/"#hallar los supercucos# conduce a", /*italian*/"il premio per chi trova #i super coccò# è", /*german*/"Deutsch"},
  });

    hintTable[GC_ROLLING_GORON_AS_CHILD] = HintText::Exclude({
                       //obscure text
                       Text{"the prize offered by a #large rolling Goron# is", /*french*/"la récompense d'un #gros Goron roulant# est", /*spanish*/"un #gran Goron rodante# otorga", /*italian*/"il premio offerto da un #grosso Goron rotolante# è", /*german*/"Deutsch"},
  });

    hintTable[LH_UNDERWATER_ITEM] = HintText::Exclude({
                       //obscure text
                       Text{"the #sunken treasure in a lake# is", /*french*/"le #trésor au fond du lac# est", /*spanish*/"el #tesoro hundido del lago# se trata de", /*italian*/"il #tesoro sommerso nel lago# è", /*german*/"Deutsch"},
  });

    hintTable[GF_GERUDO_TOKEN] = HintText::Exclude({
                       //obscure text
                       Text{"#rescuing captured carpenters# is rewarded with", /*french*/"#secourir les charpentiers capturés# assure", /*spanish*/"#rescatar los apresados carpinteros# se premia con", /*italian*/"chi salverà i #carpentieri prigionieri# riceverà", /*german*/"Deutsch"},
  });

    hintTable[WASTELAND_BOMBCHU_SALESMAN] = HintText::Exclude({
                       //obscure text
                       Text{"a #carpet guru# sells", /*french*/"#un marchand du désert# vend", /*spanish*/"el #genio de una alfombra# vende", /*italian*/"il #mercante volante# vende", /*german*/"Deutsch"},
  });


    hintTable[KAK_IMPAS_HOUSE_FREESTANDING_POH] = HintText::Exclude({
                       //obscure text
                       Text{"#imprisoned in a house# lies", /*french*/"#encagé dans une maison# gît", /*spanish*/"#en una casa entre rejas# yace", /*italian*/"in una #cella casalinga# puoi trovare", /*german*/"Deutsch"},
  });

    hintTable[HF_TEKTITE_GROTTO_FREESTANDING_POH] = HintText::Exclude({
                       //obscure text
                       Text{"#deep underwater in a hole# is", /*french*/"#dans les profondeurs d'une grotte# gît", /*spanish*/"#en lo hondo bajo un hoyo# yace", /*italian*/"in una #pozza sotterranea# puoi trovare", /*german*/"Deutsch"},
  });

    hintTable[KAK_WINDMILL_FREESTANDING_POH] = HintText::Exclude({
                       //obscure text
                       Text{"on a #windmill ledge# lies", /*french*/"#haut perché dans le moulin# gît", /*spanish*/"al #borde de un molino# yace", /*italian*/"#nel mulino, in alto#, puoi trovare", /*german*/"Deutsch"},
  });

    hintTable[GRAVEYARD_DAMPE_RACE_FREESTANDING_POH] = HintText::Exclude({
                       //obscure text
                       Text{"#racing a ghost# leads to", /*french*/"le défi du #revenant rapide# donne", /*spanish*/"#perseguir a un fantasma# conduce a", /*italian*/"#gareggiare contro un fantasma# fa vincere", /*german*/"Deutsch"},
                     }, {},
                       //clear text
                       Text{"#dead Dampe's second# prize is", /*french*/"la #deuxième course d'Igor# donne", /*spanish*/"el segundo premio de #la carrera de Dampé# se trata de", /*italian*/"la seconda ricompensa del #fantasma di Danpei# è", /*german*/"Deutsch"}
  );
    ...
    hintTable[GRAVEYARD_FREESTANDING_POH] = HintText::Exclude({
                       //obscure text
                       Text{"a #crate in a graveyard# hides", /*french*/"#la boîte dans le cimetière# contient", /*spanish*/"bajo la #caja de un cementerio# yace", /*italian*/"una #cassa nel cimitero# contiene", /*german*/"Deutsch"},
  });

    hintTable[GRAVEYARD_DAMPE_GRAVEDIGGING_TOUR] = HintText::Exclude({
                       //obscure text
                       Text{"a #gravekeeper digs up#", /*french*/"#le jeu du fossoyeur# cache", /*spanish*/"cierto #sepultero desentierra#", /*italian*/"un #becchino dissotterra#", /*german*/"Deutsch"},
  });

    hintTable[ZR_NEAR_OPEN_GROTTO_FREESTANDING_POH] = HintText::Exclude({
                       //obscure text
                       Text{"on top of a #pillar in a river# lies", /*french*/"#sur un pilier au dessus du fleuve# gît", /*spanish*/"en lo alto del #pilar de un río# yace", /*italian*/"in cima a una #colonna sul fiume# puoi trovare", /*german*/"Deutsch"},
  });

    hintTable[ZR_NEAR_DOMAIN_FREESTANDING_POH] = HintText::Exclude({
                       //obscure text
                       Text{"on a #river ledge by a waterfall# lies", /*french*/"#sur la falaise au dessus du fleuve# gît", /*spanish*/"al borde de #la entrada a una cascada# yace", /*italian*/"su una sporgenza #vicino alle cascate insonni# puoi trovare", /*german*/"Deutsch"},
  });

    hintTable[LH_FREESTANDING_POH] = HintText::Exclude({
                       //obscure text
                       Text{"high on a #lab rooftop# one can find", /*french*/"#la tour d'observation du lac# cache", /*spanish*/"en lo #alto de un laboratorio# yace", /*italian*/"sopra il #tetto di un laboratorio# qualcuno ha messo", /*german*/"Deutsch"},
  });

    hintTable[ZF_ICEBERG_FREESTANDING_POH] = HintText::Exclude({
                       //obscure text
                       Text{"#floating on ice# is", /*french*/"#gisant sur la glace# gît", /*spanish*/"#flotando sobre hielo# yace", /*italian*/"#saltare fra gli iceberg# porta verso", /*german*/"Deutsch"},
  });

    hintTable[GV_WATERFALL_FREESTANDING_POH] = HintText::Exclude({
                       //obscure text
                       Text{"behind a #desert waterfall# is", /*french*/"#derrière la cascade du désert# se cache", /*spanish*/"tras una #desierta cascada# yace", /*italian*/"#dietro una cascata# c'è una nicchia con", /*german*/"Deutsch"},
  });

    hintTable[GV_CRATE_FREESTANDING_POH] = HintText::Exclude({
                       //obscure text
                       Text{"a #crate in a valley# hides", /*french*/"la #boîte dans la vallée# contient", /*spanish*/"bajo la #caja de un valle# yace", /*italian*/"una #cassa nella valle# contiene", /*german*/"Deutsch"},
  });

    hintTable[COLOSSUS_FREESTANDING_POH] = HintText::Exclude({
                       //obscure text
                       Text{"on top of an #arch of stone# lies", /*french*/"#gisant sur une arche de pierre# gît", /*spanish*/"en lo alto de un #arco de piedra# yace", /*italian*/"in cima a #un arco di pietra# puoi trovare", /*german*/"Deutsch"},
  });

    hintTable[DMT_FREESTANDING_POH] = HintText::Exclude({
                       //obscure text
                       Text{"above a #mountain cavern entrance# is", /*french*/"gisant #au dessus de la caverne montagneuse# gît", /*spanish*/"en lo alto de la #entrada de una cueva en la montaña# yace", /*italian*/"sopra #l'ingresso di una caverna montana# puoi trovare", /*german*/"Deutsch"},
  });

    hintTable[DMC_WALL_FREESTANDING_POH] = HintText::Exclude({
                       //obscure text
                       Text{"nestled in a #volcanic wall# is", /*french*/"dans une #alcove volcanique# gît", /*spanish*/"entre unas #murallas volcánicas# yace", /*italian*/"in una #nicchia vulcanica# puoi trovare", /*german*/"Deutsch"},
  });

    hintTable[DMC_VOLCANO_FREESTANDING_POH] = HintText::Exclude({
                       //obscure text
                       Text{"obscured by #volcanic ash# is", /*french*/"#recouvert de cendres volcaniques# gît", /*spanish*/"bajo la #ceniza volcánica# yace", /*italian*/"il #fumo di un vulcano# cela", /*german*/"Deutsch"},
  });

    hintTable[GF_NORTH_F1_CARPENTER] = HintText::Exclude({
                       //obscure text
                       Text{"#defeating Gerudo guards# reveals", /*french*/"les #geôliers Gerudo# détiennent", /*spanish*/"#derrotar a las guardas Gerudo# revela", /*italian*/"una #guardia Gerudo# possiede", /*german*/"Deutsch"},
  });
    ...
    hintTable[HF_GS_NEAR_KAK_GROTTO] = HintText::Exclude({
                       //obscure text
                       Text{"a #spider-guarded spider in a hole# hoards", /*french*/"une #Skulltula dans un trou d'arachnides# a", /*spanish*/"una #Skulltula custodiada por otra# de un hoyo otorga", /*italian*/"in una grotta un #ragno protegge un'altro ragno# con", /*german*/"Deutsch"},
  });


    hintTable[LLR_GS_BACK_WALL] = HintText::Exclude({
                       //obscure text
                       Text{"night reveals a #spider in a ranch# holding", /*french*/"une #Skulltula sur la façade de la ferme# a", /*spanish*/"la noche revela una #Skulltula del rancho# que otorga", /*italian*/"#nella fattoria di notte# esce un ragno con", /*german*/"Deutsch"},
  });

    hintTable[LLR_GS_RAIN_SHED] = HintText::Exclude({
                       //obscure text
                       Text{"night reveals a #spider in a ranch# holding", /*french*/"une #Skulltula sur le mur de l'enclos# a", /*spanish*/"la noche revela una #Skulltula del rancho# que otorga", /*italian*/"#nella fattoria di notte# esce un ragno con", /*german*/"Deutsch"},
  });

    hintTable[LLR_GS_HOUSE_WINDOW] = HintText::Exclude({
                       //obscure text
                       Text{"night reveals a #spider in a ranch# holding", /*french*/"une #Skulltula sur la maison de ferme# a", /*spanish*/"la noche revela una #Skulltula del rancho# que otorga", /*italian*/"#nella fattoria di notte# esce un ragno con", /*german*/"Deutsch"},
  });

    hintTable[LLR_GS_TREE] = HintText::Exclude({
                       //obscure text
                       Text{"a spider hiding in a #ranch tree# holds", /*french*/"une #Skulltula dans l'arbre de la ferme# a", /*spanish*/"una Skulltula escondida en el #árbol de un rancho# otorga", /*italian*/"#su un albero nella fattoria# si nasconde un ragno con", /*german*/"Deutsch"},
  });


    hintTable[KF_GS_BEAN_PATCH] = HintText::Exclude({
                       //obscure text
                       Text{"a #spider buried in a forest# holds", /*french*/"une #Skulltula enterrée dans la forêt# a", /*spanish*/"una #Skulltula enterrada en un bosque# otorga", /*italian*/"un #ragno sepolto nella foresta# nasconde", /*german*/"Deutsch"},
  });
    ...
    hintTable[LW_GS_BEAN_PATCH_NEAR_BRIDGE] = HintText::Exclude({
                       //obscure text
                       Text{"a #spider buried deep in a forest maze# holds", /*french*/"une #Skulltula enterrée dans les bois# a", /*spanish*/"una #Skulltula enterrada en un laberinto forestal# otorga", /*italian*/"un #ragno sepolto in un dedalo silvestre# ha", /*german*/"Deutsch"},
  });
    ...
    hintTable[LW_GS_ABOVE_THEATER] = HintText::Exclude({
                       //obscure text
                       Text{"night reveals a #spider deep in a forest maze# holding", /*french*/"une #Skulltula haut perchée dans les bois# a", /*spanish*/"la noche revela una #Skulltula del laberinto forestal# que otorga", /*italian*/"#in un dedalo silvestre di notte# esce un ragno con", /*german*/"Deutsch"},
  });

    hintTable[SFM_GS] = HintText::Exclude({
                       //obscure text
                       Text{"night reveals a #spider in a forest meadow# holding", /*french*/"une #Skulltula dans le sanctuaire des bois# a", /*spanish*/"la noche revela una #Skulltula de la pradera del bosque# que otorga", /*italian*/"#in una radura di notte# esce un ragno con", /*german*/"Deutsch"},
  });


    hintTable[OGC_GS] = HintText::Exclude({
                       //obscure text
                       Text{"a #spider outside a tyrant's tower# holds", /*french*/"une #Skulltula parmi les ruines du château# a", /*spanish*/"una #Skulltula a las afueras de la torre de un tirano# otorga", /*italian*/"un ragno #vicino alla torre di un tiranno# ha", /*german*/"Deutsch"},
  });

    hintTable[HC_GS_TREE] = HintText::Exclude({
                       //obscure text
                       Text{"a spider hiding in a #tree outside of a castle# holds", /*french*/"une #Skulltula dans l'arbre près du château# a", /*spanish*/"una Skulltula escondida en el #árbol de las afueras de un castillo# otorga", /*italian*/"un #ragno arboreo presso un castello# nasconde", /*german*/"Deutsch"},
  });

    hintTable[MARKET_GS_GUARD_HOUSE] = HintText::Exclude({
                       //obscure text
                       Text{"a #spider in a guarded crate# holds", /*french*/"une #Skulltula dans une boîte en ville# a", /*spanish*/"una #Skulltula bajo una custodiada caja# otorga", /*italian*/"un #ragno sotto una cassa custodita# ha", /*german*/"Deutsch"},
  });
