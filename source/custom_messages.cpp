    ...
    void CreateAlwaysIncludedMessages() {
        ...
        for (u32 poes = 1; poes <= 10; poes++) {
            CreateMessage(0x9080 + poes, 0, 0, 0,
                ...
                UNSKIPPABLE()+"Eccoti bello mio! Hai un Poo per me?"+WAIT_FOR_INPUT()+NEWLINE()+UNSKIPPABLE()+"Sì???"+WAIT_FOR_INPUT()+NEWLINE()+UNSKIPPABLE()+"Fantasmagorico! Questo è un "+COLOR(QM_RED)+"Grande Poo"+COLOR(QM_WHITE)+"!"+WAIT_FOR_INPUT()+NEWLINE()+UNSKIPPABLE()
