        ...
        for(u32 shopitems = 0; shopitems < NonShopItems.size(); shopitems++) {
            ...
            if (NonShopItems[shopitems].Repurchaseable) { //Different textbox for repurchaseable items
                CreateMessage(0x9200+shopitems*2, 0, 0, 0,
                    ...
                    INSTANT_TEXT_ON()+COLOR(QM_RED)+name.GetSpanish()+": "+price+" rupias"+NEWLINE()+COLOR(QM_WHITE)+"¡Oferta especial!"+NEWLINE()+"¡Compra tantos como quieras!"+SHOP_MESSAGE_BOX()+MESSAGE_END());
            }
            else { //Normal textbox
                CreateMessage(0x9200+shopitems*2, 0, 0, 0,
