...
void AreaTable_Init_FireTemple() {
    ...
    |    VANILLA/MQ DECIDER    |
    ---------------------------*/
    areaTable[FIRE_TEMPLE_ENTRYWAY] =
        Area("Fire Temple Entryway", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {}, {},
             {
                 // Exits
    ...
    if (Dungeon::FireTemple.IsVanilla()) {
        ...
        areaTable[FIRE_TEMPLE_LOOP_ENEMIES] = Area(
            "Fire Temple Loop Enemies", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {}, {},
            {
                // Exits
                Entrance(FIRE_TEMPLE_FIRST_ROOM, { [] { return SmallKeys(FIRE_TEMPLE, 8) || !IsKeysanity; } }),
                Entrance(FIRE_TEMPLE_LOOP_TILES,
                         { [] { return Here(FIRE_TEMPLE_LOOP_ENEMIES, [] { return CanDefeatEnemies(4, 0, 15); }); } }),
            });

        areaTable[FIRE_TEMPLE_LOOP_TILES] =
            Area("Fire Temple Loop Tiles", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE,
                 {
                     // Events
                     EventAccess(&DekuBabaSticks, { [] { return DekuBabaSticks || CanGetDekuBabaSticks(4, 0, 18); } }),
                     EventAccess(&DekuBabaNuts, { [] { return DekuBabaNuts || CanGetDekuBabaNuts(4, 0, 18); } }),
                 },
                 {
                     // Locations
        ...
        areaTable[FIRE_TEMPLE_LOOP_FLARE_DANCER] =
            Area("Fire Temple Loop Flare Dancer", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {},
                 {
                     // Locations
                     LocationAccess(FIRE_TEMPLE_FLARE_DANCER_CHEST,
                                    { [] { return CanDefeatEnemy(4, 0, 3, 0) && (IsAdult || CanUse(HOOKSHOT)); },
                                      /*Glitched*/
                                      [] {
                                          return CanDefeatEnemy(4, 0, 3, 0) &&
                                                 ((CanDoGlitch(GlitchType::BombHover, GlitchDifficulty::INTERMEDIATE) &&
                                                   CanDoGlitch(GlitchType::ISG, GlitchDifficulty::INTERMEDIATE)) ||
                 ...
                 },
                 {
                     ...
                     Entrance(FIRE_TEMPLE_LOOP_HAMMER_SWITCH, { [] {
                                  return Here(FIRE_TEMPLE_LOOP_FLARE_DANCER, [] { return CanDefeatEnemy(4, 0, 3, 0); });
                              } }),
                 });

        areaTable[FIRE_TEMPLE_LOOP_HAMMER_SWITCH] =
            Area("Fire Temple Loop Hammer Switch", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE,
                 {
                     // Events
                     EventAccess(&FireLoopSwitch, { [] { return FireLoopSwitch || CanUse(MEGATON_HAMMER); },
                                                    /*Glitched*/
                                                    [] {
                                                        return CanDoGlitch(GlitchType::QPA, GlitchDifficulty::NOVICE) &&
                                                               Bombs && CanTakeDamage && CanUse(STICKS);
                                                    } }),
                 },
                 {},
                 {
                     // Exits
                     Entrance(FIRE_TEMPLE_LOOP_FLARE_DANCER, { [] { return true; } }),
                     Entrance(FIRE_TEMPLE_LOOP_GORON_ROOM,
                              { [] { return FireLoopSwitch; },
                                /*Glitched*/
                                [] { return CanDoGlitch(GlitchType::HookshotClip, GlitchDifficulty::INTERMEDIATE); } }),
                 });

        areaTable[FIRE_TEMPLE_LOOP_GORON_ROOM] =
            Area("Fire Temple Loop Goron Room", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {},
                 {
                     // Locations
        ...
        areaTable[FIRE_TEMPLE_LOOP_EXIT] =
        ...
        areaTable[FIRE_TEMPLE_BIG_LAVA_ROOM] =
            Area("Fire Temple Big Lava Room", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE,
                 {
                     // Events
                     EventAccess(&DekuBabaSticks, { [] { return DekuBabaSticks || CanGetDekuBabaSticks(4, 0, 1); } }),
                     EventAccess(&DekuBabaNuts, { [] { return DekuBabaNuts || CanGetDekuBabaNuts(4, 0, 1); } }),
                 },
                 {},
                 {
                     // Exits
                     ...
                     Entrance(FIRE_TEMPLE_BIG_LAVA_ROOM_NORTH_TILES,
                                ...
                                [] {
                                    return FireTimer >= 48 &&
                                           ((CanDoGlitch(GlitchType::BombHover, GlitchDifficulty::INTERMEDIATE) &&
                                            ...
                                            (IsAdult &&
                                             (SongOfTime &&
                                              (CanDoGlitch(GlitchType::DungeonBombOI, GlitchDifficulty::INTERMEDIATE) ||
                                               (CanDoGlitch(GlitchType::QPA, GlitchDifficulty::ADVANCED) &&
                                                (Bugs || Fish) && CanShield)))));
                                } }),
                     Entrance(FIRE_TEMPLE_BIG_LAVA_ROOM_SOUTH_GORON,
        ...
        areaTable[FIRE_TEMPLE_BIG_LAVA_ROOM_NORTH_GORON] =
        ...
        areaTable[FIRE_TEMPLE_BIG_LAVA_ROOM_NORTH_TILES] =
            Area("Fire Temple Big Lava Room North Tiles", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE,
                 {
                     // Events
                     EventAccess(&DekuBabaSticks, { [] { return DekuBabaSticks || CanGetDekuBabaSticks(4, 0, 19); } }),
                     EventAccess(&DekuBabaNuts, { [] { return DekuBabaNuts || CanGetDekuBabaNuts(4, 0, 19); } }),
                 },
                 {
                     // Locations
                     LocationAccess(FIRE_TEMPLE_GS_SONG_OF_TIME_ROOM,
                                    { [] { return CanAdultAttack || HookshotOrBoomerang; } }),
                 },
                 {
                     // Exits
                     Entrance(FIRE_TEMPLE_BIG_LAVA_ROOM, { [] { return true; } }),
        ...
        areaTable[FIRE_TEMPLE_BIG_LAVA_ROOM_SOUTH_GORON] =
            Area("Fire Temple Big Lava Room South Goron", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {},
                 {
                     // Locations
        ...
        areaTable[FIRE_TEMPLE_SHORTCUT_ROOM] = Area(
            ...
            {
                // Events
                EventAccess(&DekuBabaSticks, { [] { return DekuBabaSticks || CanGetDekuBabaSticks(4, 0, 4); } }),
                EventAccess(&DekuBabaNuts, { [] { return DekuBabaNuts || CanGetDekuBabaNuts(4, 0, 4); } }),
            },
            {
                // Locations
                LocationAccess(
                    FIRE_TEMPLE_BOULDER_MAZE_SHORTCUT_CHEST,
                    { [] { return Here(FIRE_TEMPLE_SHORTCUT_CLIMB, [] { return true; }); },
                      /*Glitched*/
                      [] { return CanDoGlitch(GlitchType::HookshotClip, GlitchDifficulty::INTERMEDIATE); } }),
            },
            {
                ...
                Entrance(FIRE_TEMPLE_SHORTCUT_CLIMB,
                         { [] { return Here(FIRE_TEMPLE_SHORTCUT_CLIMB, [] { return true; }); },
                           /*Glitched*/
                           [] {
                               return (GoronBracelet || LogicFireStrength) && Bombs && CanSurviveDamage &&
                                      CanDoGlitch(GlitchType::LedgeClip, GlitchDifficulty::INTERMEDIATE);
                           } }),
                Entrance(FIRE_TEMPLE_BOULDER_MAZE_LOWER,
                         { [] {
                              return (GoronBracelet || (IsAdult && LogicFireStrength)) &&
                                     (IsAdult || LogicFireStrength) &&
                                     (HasExplosives ||
                                      (IsAdult && (CanUse(BOW) || CanUse(HOOKSHOT) || CanUse(SLINGSHOT))));
                          },
                           /*Glitched*/
                           [] {
                               return (GoronBracelet || (IsAdult && LogicFireStrength)) &&
        ...
        areaTable[FIRE_TEMPLE_SHORTCUT_CLIMB] =
            Area("Fire Temple Shortcut Climb", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {}, {},
                 {
                     // Exits
        ...
        areaTable[FIRE_TEMPLE_BOULDER_MAZE_LOWER] = Area(
            "Fire Temple Boulder Maze Lower", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {},
            {
                // Locations
                LocationAccess(FIRE_TEMPLE_BOULDER_MAZE_LOWER_CHEST, { [] { return true; } }),
                LocationAccess(FIRE_TEMPLE_GS_BOULDER_MAZE,
                               { [] { return HasExplosives && (IsAdult || HookshotOrBoomerang); },
                                 /*Glitched*/
                                 [] {
                                     return (CanDoGlitch(GlitchType::BombHover, GlitchDifficulty::INTERMEDIATE) &&
                                             CanDoGlitch(GlitchType::ISG, GlitchDifficulty::INTERMEDIATE) && Bombs) ||
                                            Here(FIRE_TEMPLE_BOULDER_MAZE_UPPER, [] {
        ...
        areaTable[FIRE_TEMPLE_BOULDER_MAZE_LOWER_SIDE_ROOM] =
            Area("Fire Temple Boulder Maze Lower Side Room", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {},
                 {
                     // Locations
        ...
        areaTable[FIRE_TEMPLE_FIRE_WALL_CHASE] =
            Area("Fire Temple Fire Wall Chase", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE,
                 {
                     // Events
        ...
        areaTable[FIRE_TEMPLE_MAP_AREA] =
            Area("Fire Temple Map Area", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {},
                 {
                     // Locations
        ...
        areaTable[FIRE_TEMPLE_BOULDER_MAZE_UPPER] = Area(
            "Fire Temple Boulder Maze Upper", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE,
            {
                // Events
                EventAccess(&DekuBabaSticks, { [] { return DekuBabaSticks || CanGetDekuBabaSticks(4, 0, 5); } }),
                EventAccess(&DekuBabaNuts, { [] { return DekuBabaNuts || CanGetDekuBabaNuts(4, 0, 5); } }),
            },
            {
                // Locations
            ...
            },
            {
                ...
                Entrance(FIRE_TEMPLE_SCARECROW_ROOM,
                         { [] { return CanUse(SCARECROW) || (LogicFireScarecrow && IsAdult && CanUse(LONGSHOT)); } }),
            });

        areaTable[FIRE_TEMPLE_SCARECROW_ROOM] = Area(
            "Fire Temple Scarecrow Room", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {},
            {
                // Locations
        ...
        areaTable[FIRE_TEMPLE_EAST_PEAK] =
            Area("Fire Temple East Peak", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {},
                 {
                     // Locations
        ...
        areaTable[FIRE_TEMPLE_CORRIDOR] =
            Area("Fire Temple Corridor", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {}, {},
                 {
                     // Exits
        ...
        areaTable[FIRE_TEMPLE_FIRE_MAZE_UPPER] = Area(
            "Fire Temple Fire Maze Upper", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {}, {},
            {
                ...
                Entrance(FIRE_TEMPLE_FIRE_MAZE_ROOM, { [] { return true; } }),
                Entrance(FIRE_TEMPLE_WEST_CENTRAL_UPPER,
                         { [] { return CanUse(MEGATON_HAMMER); },
                           /*Glitched*/
                           [] { return CanUse(STICKS) && CanDoGlitch(GlitchType::QPA, GlitchDifficulty::NOVICE); } }),
            });

        areaTable[FIRE_TEMPLE_FIRE_MAZE_SIDE_ROOM] =
        ...
        areaTable[FIRE_TEMPLE_WEST_CENTRAL_LOWER] = Area(
            "Fire Temple West Central Lower", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {},
            {
                // Locations
                LocationAccess(
                    FIRE_TEMPLE_HIGHEST_GORON_CHEST,
                    { [] {
                         return Here(FIRE_TEMPLE_WEST_CENTRAL_UPPER,
                                     [] { return CanPlay(SongOfTime) && CanUse(MEGATON_HAMMER); });
                     },
                      /*Glitched*/
                      [] {
                          return CanDoGlitch(GlitchType::HookshotClip, GlitchDifficulty::INTERMEDIATE) ||
                                 Here(FIRE_TEMPLE_WEST_CENTRAL_UPPER, [] {
                                     return (IsAdult || CanDoGlitch(GlitchType::Megaflip, GlitchDifficulty::NOVICE) ||
                                             (Bombs && HasBombchus &&
                                              CanDoGlitch(GlitchType::BombHover, GlitchDifficulty::NOVICE) &&
                                              CanDoGlitch(GlitchType::ISG, GlitchDifficulty::INTERMEDIATE))) &&
                                            (((SongOfTime &&
                                               (((Bugs || Fish) && CanShield &&
                                                 CanDoGlitch(GlitchType::QPA, GlitchDifficulty::ADVANCED)) ||
                                                CanDoGlitch(GlitchType::DungeonBombOI, GlitchDifficulty::NOVICE))) ||
                                              CanPlay(SongOfTime)) &&
                                             ((CanUse(STICKS) &&
            ...
            },
            {
                ...
                Entrance(FIRE_TEMPLE_WEST_CENTRAL_UPPER,
                         { [] { return IsAdult && CanPlay(SongOfTime); },
                           /*Glitched*/
                           [] {
                               return (IsAdult && SongOfTime &&
                                       (((Bugs || Fish) && CanShield &&
                                         (CanSurviveDamage || (CanTakeDamage && NumBottles >= 2)) && Bombs &&
                                         CanDoGlitch(GlitchType::QPA, GlitchDifficulty::ADVANCED)) ||
                                        (CanDoGlitch(GlitchType::DungeonBombOI, GlitchDifficulty::NOVICE)))) ||
                                      (Bombs && CanDoGlitch(GlitchType::BombHover, GlitchDifficulty::INTERMEDIATE) &&
                                       CanDoGlitch(GlitchType::ISG, GlitchDifficulty::INTERMEDIATE));
                           } }),
                Entrance(FIRE_TEMPLE_LATE_FIRE_MAZE, { [] { return true; } }),
            });

        areaTable[FIRE_TEMPLE_WEST_CENTRAL_UPPER] = Area(
            "Fire Temple West Central Upper", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {}, {},
            {
                // Exits
                Entrance(
        ...
        areaTable[FIRE_TEMPLE_LATE_FIRE_MAZE] = Area(
            "Fire Temple Late Fire Maze", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE,
            {
                // Events
                EventAccess(&DekuBabaSticks,
                            { [] { return DekuBabaSticks || CanGetDekuBabaSticks(4, 0, 10, { 14 }); } }),
                EventAccess(&DekuBabaNuts, { [] { return DekuBabaNuts || CanGetDekuBabaNuts(4, 0, 10, { 14 }); } }),
            },
            {},
            {
                // Exits
                Entrance(FIRE_TEMPLE_FIRE_MAZE_ROOM,
                         { [] { return false; },
                           /*Glitched*/
                           [] {
                               return CanDoGlitch(GlitchType::Megaflip, GlitchDifficulty::NOVICE) ||
                                      (Bombs && CanShield &&
                                       CanDoGlitch(GlitchType::SuperSlide, GlitchDifficulty::ADVANCED));
                           } }),
                Entrance(FIRE_TEMPLE_WEST_CENTRAL_LOWER, { [] { return true; } }),
                Entrance(FIRE_TEMPLE_UPPER_FLARE_DANCER, { [] { return HasExplosives; } }),
            });

        areaTable[FIRE_TEMPLE_UPPER_FLARE_DANCER] = Area(
            "Fire Temple Upper Flare Dancer", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {}, {},
            {
                // Exits
                Entrance(FIRE_TEMPLE_LATE_FIRE_MAZE, { [] {
                             return Here(FIRE_TEMPLE_UPPER_FLARE_DANCER, [] { return CanDefeatEnemy(4, 0, 24, 0); });
                         } }),
        ...
        areaTable[FIRE_TEMPLE_WEST_CLIMB] = Area(
            "Fire Temple West Climb", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {}, {},
            {
                // Exits
                Entrance(FIRE_TEMPLE_UPPER_FLARE_DANCER, { [] { return true; } }),
                Entrance(FIRE_TEMPLE_WEST_PEAK,
                         { [] { return CanUseProjectile; },
                           /*Glitched*/ [] { return CanDoGlitch(GlitchType::SuperStab, GlitchDifficulty::NOVICE); } }),
            });

        areaTable[FIRE_TEMPLE_WEST_PEAK] =
            Area("Fire Temple West Peak", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE,
                 {
                     // Events
                     EventAccess(&DekuBabaSticks, { [] { return DekuBabaSticks || CanGetDekuBabaSticks(4, 0, 13); } }),
                     EventAccess(&DekuBabaNuts, { [] { return DekuBabaNuts || CanGetDekuBabaNuts(4, 0, 13); } }),
                 },
                 {
                     // Locations
        ...
        areaTable[FIRE_TEMPLE_HAMMER_RETURN_PATH] =
            Area("Fire Temple Hammer Return Path", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE,
                 {
                     // Events
                     EventAccess(&DekuBabaSticks, { [] { return DekuBabaSticks || CanGetDekuBabaSticks(4, 0, 14); } }),
                     EventAccess(&DekuBabaNuts, { [] { return DekuBabaNuts || CanGetDekuBabaNuts(4, 0, 14); } }),
                 },
                 {},
                 {
                     // Exits
        ...
        areaTable[FIRE_TEMPLE_ABOVE_FIRE_MAZE] =
    ...
    if (Dungeon::FireTemple.IsMQ()) {
        areaTable[FIRE_TEMPLE_MQ_LOWER] = Area(
            "Fire Temple MQ Lower", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {},
            {
                // Locations
                LocationAccess(FIRE_TEMPLE_MQ_MAP_ROOM_SIDE_CHEST, { [] {
                                   return CanJumpslash || CanUse(SLINGSHOT) || CanUse(BOW) || Bombs ||
                                          CanUse(DINS_FIRE);
                               } }),
                                          ...
                                          (CanUse(FIRE_ARROWS) ||
                                           (CanUse(DINS_FIRE) &&
                                            ((DamageMultiplier.IsNot(DAMAGEMULTIPLIER_OHKO) &&
                                              ...
                                              DamageMultiplier.IsNot(DAMAGEMULTIPLIER_OCTUPLE) &&
                                              DamageMultiplier.IsNot(DAMAGEMULTIPLIER_SEXDECUPLE)) ||
                                             CanUse(GORON_TUNIC) || CanUse(BOW) || CanUse(LONGSHOT))));
                               } }),
                // Trick: IsAdult && (LogicFewerTunicRequirements || CanUse(GORON_TUNIC)) && (((CanUse(HOVER_BOOTS) ||
                // (LogicFireMQNearBoss && CanUse(BOW))) && HasFireSource) || (CanUse(HOOKSHOT) && CanUse(FIRE_ARROWS)
                // || (CanUse(DINS_FIRE) && ((DamageMultiplier.IsNot(DAMAGEMULTIPLIER_OHKO) &&
                // DamageMultiplier.IsNot(DAMAGEMULTIPLIER_QUADRUPLE) &&
                // DamageMultiplier.IsNot(DAMAGEMULTIPLIER_OCTUPLE) &&
                // DamageMultiplier.IsNot(DAMAGEMULTIPLIER_SEXDECUPLE)) || CanUse(GORON_TUNIC) || CanUse(BOW) ||
                // CanUse(LONGSHOT)))))
            },
            {
                // Exits
                Entrance(FIRE_TEMPLE_ENTRYWAY, { [] { return true; } }),
                Entrance(FIRE_TEMPLE_BOSS_ENTRYWAY, { [] {
                             return IsAdult && CanUse(GORON_TUNIC) && CanUse(MEGATON_HAMMER) && BossKeyFireTemple &&
                                    ((HasFireSource && (LogicFireBossDoorJump || HoverBoots)) ||
                                     HasAccessTo(FIRE_TEMPLE_MQ_UPPER));
                         } }),
                Entrance(FIRE_TEMPLE_MQ_LOWER_LOCKED_DOOR, { [] {
                             return SmallKeys(FIRE_TEMPLE, 5) &&
        ...
        areaTable[FIRE_TEMPLE_MQ_LOWER_LOCKED_DOOR] =
            Area("Fire Temple MQ Lower Locked Door", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE,
                 ...
                 {
                     // Locations
                     LocationAccess(FIRE_TEMPLE_MQ_MEGATON_HAMMER_CHEST, { [] {
                                        return IsAdult && (CanUse(KOKIRI_SWORD) || MasterSword || BiggoronSword) &&
                                               (HasExplosives || Hammer || Hookshot);
                                    } }),
                     LocationAccess(FIRE_TEMPLE_MQ_MAP_CHEST, { [] { return IsAdult && CanUse(MEGATON_HAMMER); } }),
                 },
                 {});

        areaTable[FIRE_TEMPLE_MQ_BIG_LAVA_ROOM] = Area(
            "Fire Temple MQ Big Lava Room", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE,
            {
                // Events
                EventAccess(&FairyPot, { [] {
                    return FairyPot || (HasFireSource && Bow && IsAdult && (CanUse(HOOKSHOT) || LogicFireSongOfTime));
                } }),
            ...
            {
                // Locations
                LocationAccess(FIRE_TEMPLE_MQ_BOSS_KEY_CHEST,
                               { [] { return HasFireSource && Bow && IsAdult && CanUse(HOOKSHOT); } }),
                // Trick: HasFireSource && (Bow || LogicFireMQBKChest) && IsAdult && CanUse(HOOKSHOT)
                LocationAccess(FIRE_TEMPLE_MQ_BIG_LAVA_ROOM_BLOCKED_DOOR_CHEST,
                               { [] { return HasFireSource && HasExplosives && IsAdult && CanUse(HOOKSHOT); } }),
            ...
            {
                // Exits
                Entrance(FIRE_TEMPLE_MQ_LOWER_MAZE, { [] {
                             return IsAdult && CanUse(GORON_TUNIC) && SmallKeys(FIRE_TEMPLE, 2) && HasFireSource;
                         } }),
                // Trick: IsAdult && CanUse(GORON_TUNIC) && SmallKeys(FIRE_TEMPLE, 2) && (HasFireSource ||
                // (LogicFireMQClimb && HoverBoots))
            });

        areaTable[FIRE_TEMPLE_MQ_LOWER_MAZE] = Area(
            "Fire Temple MQ Lower Maze", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {},
            {
                // Locations
                LocationAccess(FIRE_TEMPLE_MQ_LIZALFOS_MAZE_LOWER_CHEST,
                               { [] { return CanUse(MASTER_SWORD) || CanUse(BIGGORON_SWORD); } }),
                LocationAccess(FIRE_TEMPLE_MQ_LIZALFOS_MAZE_SIDE_ROOM_CHEST,
                               { [] { return HasExplosives && HasAccessTo(FIRE_TEMPLE_MQ_UPPER_MAZE); } }),
            ...
            },
            {
                ...
                // Trick: (IsAdult && ((HasExplosives && CanUse(HOOKSHOT)) || (LogicFireMQMazeHovers &&
                // CanUse(HOVER_BOOTS)))) || LogicFireMQMazeJump
            });

        areaTable[FIRE_TEMPLE_MQ_UPPER_MAZE] =
            Area("Fire Temple MQ Upper Maze", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE,
                 {
                     // Events
                     // EventAccess(&WallFairy, {[]{return WallFairy || (IsAdult && (((CanPlay(SongOfTime) &&
                     // CanUse(HOOKSHOT) && HasExplosives) || CanUse(LONGSHOT))));}}),
                 ...
                 },
                 {
                     ...
                     LocationAccess(FIRE_TEMPLE_MQ_GS_SKULL_ON_FIRE, { [] {
                                        return IsAdult && ((CanPlay(SongOfTime) && CanUse(HOOKSHOT) && HasExplosives) ||
                                                           CanUse(LONGSHOT));
                                    } }),
                 },
                 {
                     // Exits
                     Entrance(FIRE_TEMPLE_MQ_UPPER, { [] {
                                  return SmallKeys(FIRE_TEMPLE, 3) && IsAdult &&
                                         ((CanUse(BOW) && CanUse(HOOKSHOT)) || CanUse(FIRE_ARROWS));
                              } }),
                 });

        areaTable[FIRE_TEMPLE_MQ_UPPER] =
            Area("Fire Temple MQ Upper", "Fire Temple", FIRE_TEMPLE, NO_DAY_NIGHT_CYCLE, {},
                 {
                     // Locations
                     LocationAccess(FIRE_TEMPLE_MQ_FREESTANDING_KEY, { [] {
                                        return (CanUse(KOKIRI_SWORD) || CanUse(MASTER_SWORD) ||
                                                CanUse(BIGGORON_SWORD) || CanUse(SLINGSHOT) || CanUse(BOOMERANG)) &&
                                               CanUse(HOOKSHOT);
                                    } }),
                     // Trick: (IsAdult && CanUse(HOOKSHOT)) || LogicFireMQFlameMaze
                     LocationAccess(FIRE_TEMPLE_MQ_CHEST_ON_FIRE,
                                    { [] { return IsAdult && CanUse(HOOKSHOT) && SmallKeys(FIRE_TEMPLE, 4); } }),
                     // Trick: ((IsAdult && CanUse(HOOKSHOT)) || LogicFireMQFlameMaze) && SmallKeys(FIRE_TEMPLE, 4)
                     LocationAccess(FIRE_TEMPLE_MQ_GS_FIRE_WALL_MAZE_SIDE_ROOM,
                                    { [] { return CanPlay(SongOfTime) || HoverBoots; } }),
                     ...
                     LocationAccess(FIRE_TEMPLE_MQ_GS_ABOVE_FIRE_WALL_MAZE,
                                    { [] { return IsAdult && CanUse(HOOKSHOT) && SmallKeys(FIRE_TEMPLE, 5); } }),
                     // Trick: (IsAdult && CanUse(HOOKSHOT) && SmallKeys(FIRE_TEMPLE, 5)) || (LogicFireMQAboveMazeGS &&
    ...
    areaTable[FIRE_TEMPLE_BOSS_ROOM] = Area(
