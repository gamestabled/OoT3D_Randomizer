...
void AreaTable_Init_Kakariko() {
    ...
    areaTable[KAKARIKO_VILLAGE] = Area(
        "Kakariko Village", "Kakariko Village", KAKARIKO_VILLAGE, NO_DAY_NIGHT_CYCLE,
        {
            // Events
            EventAccess(&CojiroAccess, { [] { return CojiroAccess || (IsAdult && WakeUpAdultTalon); } }),
            EventAccess(&BugRock, { [] { return true; } }),
            EventAccess(&KakarikoVillageGateOpen, { [] {
                return KakarikoVillageGateOpen || (IsChild && (ZeldasLetter || OpenKakariko.Is(OPENKAKARIKO_OPEN)));
            } }),
        },
        {
            // Locations
        ...
        },
        {
            ...
            Entrance(BOTTOM_OF_THE_WELL_ENTRYWAY,
                     { [] { return DrainWell && (IsChild || ShuffleDungeonEntrances.IsNot(SHUFFLEDUNGEONS_OFF)); },
                       /*Glitched*/
                       [] {
                           return (IsChild &&
                                   (CanDoGlitch(GlitchType::TripleSlashClip, GlitchDifficulty::NOVICE) ||
                                    (AtNight &&
                                     (CanDoGlitch(GlitchType::NaviDive_Stick, GlitchDifficulty::NOVICE) ||
                                  ...
                                  (IsAdult && CanUse(LONGSHOT) && AtDay &&
                                   CanDoGlitch(GlitchType::TripleSlashClip, GlitchDifficulty::NOVICE));
                       } }),
            Entrance(KAK_POTION_SHOP_FRONT, { [] { return AtDay || IsChild; } }),
            Entrance(KAK_REDEAD_GROTTO, { [] { return CanOpenBombGrotto; } }),
            Entrance(KAK_IMPAS_LEDGE,
                     { [] { return (IsChild && AtDay) || CanUse(HOOKSHOT); },
                       /*Glitched*/
                       [] {
                           return IsAdult &&
                                  ((HoverBoots && CanDoGlitch(GlitchType::Megaflip, GlitchDifficulty::INTERMEDIATE)) ||
                                   (Bombs && HasBombchus &&
            ...
            Entrance(KAK_IMPAS_ROOFTOP,
                     { [] { return CanUse(HOOKSHOT); },
                       /*Glitched*/
                       [] {
                           return (HoverBoots && CanDoGlitch(GlitchType::Megaflip, GlitchDifficulty::INTERMEDIATE)) ||
                                  (Bombs && HasBombchus &&
                                   CanDoGlitch(GlitchType::BombHover, GlitchDifficulty::INTERMEDIATE)) ||
                                  CanDoGlitch(GlitchType::HoverBoost, GlitchDifficulty::INTERMEDIATE);
                       } }),
            ...
            Entrance(
                KAK_BEHIND_GATE,
                { [] { return IsAdult || (KakarikoVillageGateOpen); },
                  /*Glitched*/ [] { return CanDoGlitch(GlitchType::BombHover, GlitchDifficulty::INTERMEDIATE); } }),
        });

    areaTable[KAK_IMPAS_LEDGE] =
        Area("Kak Impas Ledge", "Kakariko Village", KAKARIKO_VILLAGE, NO_DAY_NIGHT_CYCLE, {}, {},
             {
                 // Exits
    ...
    areaTable[KAK_BACKYARD] = Area(
        "Kak Backyard", "Kakariko Village", KAKARIKO_VILLAGE, NO_DAY_NIGHT_CYCLE, {}, {},
        {
            // Exits
            Entrance(KAKARIKO_VILLAGE, { [] { return true; } }),
        ...
        Area("Kak Impas House Near Cow", "Kak Impas House", NONE, NO_DAY_NIGHT_CYCLE, {},
             {
                 ...
                 LocationAccess(KAK_IMPAS_HOUSE_COW,
                                { [] { return CanPlay(EponasSong); },
                                  /*Glitched*/
                                  [] {
                                      return (CanDoGlitch(GlitchType::IndoorBombOI, GlitchDifficulty::ADVANCED) ||
                                              ((Bugs || Fish) && CanShield &&
    ...
    areaTable[KAK_WINDMILL] = Area(
        "Kak Windmill", "Windmill and Dampes Grave", NONE, NO_DAY_NIGHT_CYCLE,
        {
            ...
            EventAccess(&DrainWell,
                        { [] { return DrainWell || (IsChild && CanPlay(SongOfStorms)); },
                          /*Glitched*/
                          [] {
                              return IsChild && SongOfStorms &&
                                     (CanDoGlitch(GlitchType::WindmillBombOI, GlitchDifficulty::ADVANCED) ||
                                      ((Fish || Bugs) && CanShield &&
                                       ((Bombs && (CanSurviveDamage || (Fairy && NumBottles >= 2))) ||
                                        CanDoGlitch(GlitchType::BombHover, GlitchDifficulty::INTERMEDIATE)) &&
                                       CanDoGlitch(GlitchType::QPA, GlitchDifficulty::ADVANCED)) ||
                                      ((Fish || Bugs) && HasBombchus && CanShield &&
        ...
        },
        {
            ...
            LocationAccess(
                ...
                { [] {
                     return CanUse(BOOMERANG) || DampesWindmillAccess ||
                            (IsAdult && CanJumpslash && CanUse(HOOKSHOT) && LogicWindmillPoHHookshot);
                 },
                  /*Glitched*/ [] { return CanDoGlitch(GlitchType::BombHover, GlitchDifficulty::INTERMEDIATE); } }),
            LocationAccess(SONG_FROM_WINDMILL,
                           { [] { return IsAdult && Ocarina; },
                             /*Glitched*/
                             [] {
                                 return IsAdult &&
                                        (CanDoGlitch(GlitchType::WindmillBombOI, GlitchDifficulty::EXPERT) ||
                                         ((Fish || Bugs) && CanShield &&
                                          ((Bombs && (CanSurviveDamage || (Fairy && NumBottles >= 2))) ||
    ...
    areaTable[THE_GRAVEYARD] = Area(
        ...
        },
        {
            ...
            Entrance(GRAVEYARD_DAMPES_HOUSE,
                     { [] { return IsAdult || AtDampeTime; } }), // TODO: This needs to be handled
            Entrance(KAKARIKO_VILLAGE, { [] { return true; } }),
        ...
        "Graveyard Dampes Grave", "Windmill and Dampes Grave", NONE, NO_DAY_NIGHT_CYCLE,
        {
            ...
            EventAccess(&DampesWindmillAccess,
                        { [] { return DampesWindmillAccess || (IsAdult && CanPlay(SongOfTime)); },
                          /*Glitched*/
                          [] {
                              return (CanDoGlitch(GlitchType::WindmillBombOI, GlitchDifficulty::INTERMEDIATE) ||
                                      ((Bugs || Fish) && CanShield &&
                                       (CanSurviveDamage || (Fairy && NumBottles >= 2)) &&
                                       CanDoGlitch(GlitchType::QPA, GlitchDifficulty::ADVANCED)) ||
                                      ((Bugs || Fish) && CanShield && HasBombchus &&
    ...
    areaTable[GRAVEYARD_WARP_PAD_REGION] = Area(
        ...
        },
        {
            ...
            Entrance(
                ...
                { [] { return CanUse(DINS_FIRE) || (LogicShadowFireArrowEntry && IsAdult && CanUse(FIRE_ARROWS)); },
                  /*Glitched*/
                  [] {
                      return (CanUse(STICKS) &&
                              (CanDoGlitch(GlitchType::QPA, GlitchDifficulty::INTERMEDIATE) || CanUse(FIRE_ARROWS))) ||
                             (CanTakeDamage &&
                              (CanDoGlitch(GlitchType::BombHover, GlitchDifficulty::INTERMEDIATE) ||
                               CanDoGlitch(GlitchType::HookshotJump_Bonk, GlitchDifficulty::INTERMEDIATE) ||
                               CanDoGlitch(GlitchType::HookshotJump_Boots, GlitchDifficulty::NOVICE)));
    ...
    areaTable[KAK_BEHIND_GATE] = Area("Kak Behind Gate", "Kakariko Village", NONE, NO_DAY_NIGHT_CYCLE, {}, {},
                                      {
                                          ...
                                          Entrance(KAKARIKO_VILLAGE, { [] {
                                                       return IsAdult || LogicVisibleCollision ||
                                                              KakarikoVillageGateOpen ||
                                                              OpenKakariko.Is(OPENKAKARIKO_OPEN);
                                                   } }),
