...
void AreaTable_Init_HyruleField() {
    areaTable[HYRULE_FIELD] = Area(
        ...
        },
        {
            ...
            Entrance(HF_COW_GROTTO, { [] { return (CanUse(MEGATON_HAMMER) || IsChild) && CanOpenBombGrotto; },
                                      /*Glitched*/
                                      [] {
                                          return (CanUse(STICKS) && (ShardOfAgony || LogicGrottosWithoutAgony) &&
                                                  (IsChild ||
                                                   (CanOpenBombGrotto &&
                                                    (CanTakeDamageTwice ||
                                                     CanDoGlitch(GlitchType::QPA, GlitchDifficulty::ADVANCED)))) &&
            ...
            Entrance(HF_FAIRY_GROTTO, { [] { return Here(HYRULE_FIELD, [] { return CanBlastOrSmash; }); },
                                        ...
                                        [] {
                                            return Here(HYRULE_FIELD, [] {
                                                return CanUse(STICKS) &&
                                                       ((IsChild && CanTakeDamage &&
                                                         CanDoGlitch(GlitchType::QPA, GlitchDifficulty::EXPERT)) ||
                                                        (IsAdult &&
                                                         CanDoGlitch(GlitchType::QPA, GlitchDifficulty::ADVANCED)));
                                            });
                                        } }),
            Entrance(HF_NEAR_KAK_GROTTO,
                     { [] { return CanOpenBombGrotto; },
                       /*Glitched*/
                       [] { return Sticks && IsChild && CanDoGlitch(GlitchType::QPA, GlitchDifficulty::ADVANCED); } }),
            Entrance(HF_TEKTITE_GROTTO,
                     { [] { return CanOpenBombGrotto; },
                       /*Glitched*/
                       [] { return Sticks && IsChild && CanDoGlitch(GlitchType::QPA, GlitchDifficulty::ADVANCED); } }),
        });
    ...
    areaTable[HF_COW_GROTTO] = Area(
        "HF Cow Grotto", "", NONE, NO_DAY_NIGHT_CYCLE, grottoEvents,
        {
            // Locations
    ...
    areaTable[HF_NEAR_MARKET_GROTTO] =
        Area("HF Near Market Grotto", "", NONE, NO_DAY_NIGHT_CYCLE, grottoEvents,
             {
                 // Locations
    ...
    areaTable[HF_NEAR_KAK_GROTTO] =
        Area("HF Near Kak Grotto", "", NONE, NO_DAY_NIGHT_CYCLE, {},
             {
                 // Locations
    ...
    areaTable[HF_TEKTITE_GROTTO] =
        Area("HF Tektite Grotto", "", NONE, NO_DAY_NIGHT_CYCLE, {},
             {
                 // Locations
                 LocationAccess(HF_TEKTITE_GROTTO_FREESTANDING_POH,
                                { [] { return ProgressiveScale >= 2 || CanUse(IRON_BOOTS); } }),
             },
             {
                 // Exits
                 Entrance(HYRULE_FIELD, { [] { return true; } }),
    ...
    areaTable[LAKE_HYLIA] = Area(
        ...
        },
        {
            ...
            LocationAccess(LH_FREESTANDING_POH,
                           { [] { return IsAdult && (CanUse(SCARECROW) || CanPlantBean(LAKE_HYLIA)); },
                             /*Glitched*/
                             [] {
                                 return (IsAdult && CanUse(HOOKSHOT) && ScarecrowSong &&
                                         (CanDoGlitch(GlitchType::OutdoorBombOI, GlitchDifficulty::INTERMEDIATE) ||
                                          ((Bugs || Fish) && CanShield &&
                                           CanDoGlitch(GlitchType::QPA, GlitchDifficulty::ADVANCED)) ||
                                          ((Bugs || Fish) &&
        ...
        },
        {
            ...
            Entrance(ZORAS_DOMAIN,
                     { [] { return IsChild && (CanDive || CanUse(IRON_BOOTS)); },
                       /*Glitched*/
                       [] {
                           return IsChild &&
                                  (CanDoGlitch(GlitchType::TripleSlashClip, GlitchDifficulty::NOVICE) ||
                                   CanDoGlitch(GlitchType::NaviDive_Stick, GlitchDifficulty::INTERMEDIATE) ||
                                   (Bugs && CanUse(HOVER_BOOTS) &&
            ...
            Entrance(LH_FISHING_ISLAND,
                     { [] { return IsChild || CanUse(SCARECROW) || CanPlantBean(LAKE_HYLIA) || WaterTempleClear; },
                       /*Glitched*/
                       [] {
                           return (CanUse(HOOKSHOT) && ScarecrowSong &&
                                   (CanDoGlitch(GlitchType::OutdoorBombOI, GlitchDifficulty::INTERMEDIATE) ||
                                    ((Bugs || Fish) && CanShield &&
                                     CanDoGlitch(GlitchType::QPA, GlitchDifficulty::ADVANCED)) ||
                                    ((Bugs || Fish) &&
            ...
            Entrance(WATER_TEMPLE_ENTRYWAY,
                     { [] {
                          return CanUse(HOOKSHOT) &&
                                 (CanUse(IRON_BOOTS) || (IsAdult && CanUse(LONGSHOT) && ProgressiveScale >= 2));
                      },
    ...
    areaTable[LH_LAB] = Area(
        "LH Lab", "", NONE, NO_DAY_NIGHT_CYCLE,
        {
            // Events
            EventAccess(&EyedropsAccess, { [] {
                return EyedropsAccess || (IsAdult && (EyeballFrogAccess || (EyeballFrog && DisableTradeRevert)));
            } }),
        },
        {
            // Locations
            LocationAccess(
                LH_LAB_DIVE,
                { [] { return ProgressiveScale >= 2 || (LogicLabDiving && CanUse(IRON_BOOTS) && CanUse(HOOKSHOT)); },
                  /*Glitched*/
                  [] {
                      return CanDoGlitch(GlitchType::TripleSlashClip, GlitchDifficulty::INTERMEDIATE) ||
    ...
    areaTable[LLR_TALONS_HOUSE] =
        Area("LLR Talons House", "", NONE, NO_DAY_NIGHT_CYCLE, {},
             {
                 // Locations
    ...
    areaTable[LLR_STABLES] =
        Area("LLR Stables", "", NONE, NO_DAY_NIGHT_CYCLE, {},
             {
                 // Locations
                 LocationAccess(LLR_STABLES_LEFT_COW,
    ...
    areaTable[LLR_TOWER] =
        Area("LLR Tower", "", NONE, NO_DAY_NIGHT_CYCLE, {},
             {
                 // Locations
                 LocationAccess(LLR_FREESTANDING_POH, { [] { return IsChild; } }),
                 LocationAccess(LLR_TOWER_LEFT_COW,
                                { [] { return CanPlay(EponasSong); },
                                  /*Glitched*/
                                  [] {
                                      return (CanDoGlitch(GlitchType::IndoorBombOI, GlitchDifficulty::EXPERT) ||
                 ...
                 LocationAccess(LLR_TOWER_RIGHT_COW,
                                { [] { return CanPlay(EponasSong); },
                                  /*Glitched*/
                                  [] {
                                      return (CanDoGlitch(GlitchType::IndoorBombOI, GlitchDifficulty::EXPERT) ||
