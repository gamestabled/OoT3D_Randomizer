/* Edited from
 * https://github.com/devkitPro/libctru/blob/846b79e5a652dd00df38bd23bf9e99d519e3171a/libctru/source/services/uds.c */
...
static Result udsipc_BeginHostingNetwork(const udsNetworkStruct* network, const void* passphrase,
                                         size_t passphrase_size);
static Result udsipc_ConnectToNetwork(const udsNetworkStruct* network, const void* passphrase, size_t passphrase_size,
                                      udsConnectionType connection_type);
static Result udsipc_SetProbeResponseParam(u32 oui, s8 data);

static Result udsipc_RecvBeaconBroadcastData(u8* outbuf, u32 maxsize, nwmScanInputStruct* scaninput, u32 wlancommID,
                                             u8 id8, Handle event);
...
Result udsInit(size_t sharedmem_size, const char* username) {
    ...
    if (AtomicPostIncrement(&__uds_refcount))
        return 0;
...
void udsExit(void) {
    if (AtomicDecrement(&__uds_refcount))
        return;
...
void udsGenerateDefaultNetworkStruct(udsNetworkStruct* network, u32 wlancommID, u8 id8, u8 max_nodes) {
    ...
    if (max_nodes > UDS_MAXNODES)
        max_nodes = UDS_MAXNODES;
...
static Result uds_Initialize(u32 sharedmem_size, const char* username) {
    ...
    if (__uds_sharedmem_addr == NULL)
        ret = -1;

    if (R_SUCCEEDED(ret)) {
        memset(__uds_sharedmem_addr, 0, __uds_sharedmem_size);
        ret = svcCreateMemoryBlock(&__uds_sharedmem_handle, (u32)__uds_sharedmem_addr, __uds_sharedmem_size, 0x0,
                                   MEMPERM_READ | MEMPERM_WRITE);
    }

    if (R_SUCCEEDED(ret))
        ret = udsipc_InitializeWithVersion(&nodeinfo, __uds_sharedmem_handle, __uds_sharedmem_size,
...
Result udsCreateNetwork(const udsNetworkStruct* network, const void* passphrase, size_t passphrase_size,
    ...
    if (context)
        ret = udsBind(context, UDS_BROADCAST_NETWORKNODEID, false, data_channel, recv_buffer_size);
    if (R_FAILED(ret))
        return ret;
...
Result udsConnectNetwork(const udsNetworkStruct* network, const void* passphrase, size_t passphrase_size,
    ...
    if (connection_type == UDSCONTYPE_Spectator)
        spectator = true;

    if (context)
        ret = udsBind(context, recv_NetworkNodeID, spectator, data_channel, recv_buffer_size);
    if (R_FAILED(ret))
        return ret;
...
static Result udsipc_InitializeWithVersion(udsNodeInfo* nodeinfo, Handle sharedmem_handle, u32 sharedmem_size,
    ...
    Result ret = 0;
    if (R_FAILED(ret = svcSendSyncRequest(__uds_servhandle)))
        return ret;
    ret = cmdbuf[1];
...
Result udsSetNewConnectionsBlocked(bool block, bool clients, bool flag) {
    ...
    if (clients)
        bitmask |= UDSNETATTR_DisableConnectClients;
    if (flag)
        bitmask |= UDSNETATTR_x4;
...
Result udsGetConnectionStatus(udsConnectionStatus* output) {
    ...
    if (R_SUCCEEDED(ret)) {
        if (output)
            memcpy(output, &cmdbuf[2], sizeof(udsConnectionStatus));
    }
...
bool udsWaitConnectionStatusEvent(bool nextEvent, bool wait) {
    ...
    if (nextEvent)
        svcClearEvent(__uds_connectionstatus_event);

    if (svcWaitSynchronization(__uds_connectionstatus_event, delayvalue) != 0 && !wait)
        ret = false;
...
Result udsGetNodeInformation(u16 NetworkNodeID, udsNodeInfo* output) {
    ...
    if (R_SUCCEEDED(ret)) {
        if (output)
            memcpy(output, &cmdbuf[2], sizeof(udsNodeInfo));
    }

    return ret;
}

Result udsScanBeacons(void* buf, size_t maxsize, udsNetworkScanInfo** networks, size_t* total_networks, u32 wlancommID,
                      u8 id8, const u8* host_macaddress, bool connected) {
    Result ret   = 0;
    Handle event = 0;
    u8* outbuf   = (u8*)buf;
    u32 entpos, curpos;
    ...
    if (host_macaddress)
        memcpy(scaninput.mac_address, host_macaddress, sizeof(scaninput.mac_address));

    if (maxsize < sizeof(nwmBeaconDataReplyHeader))
        return -2;
    ...
    if (connected)
        ret = udsipc_ScanOnConnection(outbuf, maxsize, &scaninput, wlancommID, id8);
    ...
    if (hdr->total_entries) {
        if (networks) {
            ...
            if (networks_ptr == NULL)
                return -1;
            if (total_networks)
                *total_networks = hdr->total_entries;
            ...
            *networks = networks_ptr;
            ...
            for (entpos = 0; entpos < hdr->total_entries; entpos++) {
                ...
                if (entry->size > hdr->size || curpos + entry->size > hdr->size ||
                    entry->size <= sizeof(nwmBeaconDataReplyEntry)) {
                    ret = -2;
                    break;
                }
                ...
                ret = usd_parsebeacon(&outbuf[curpos + sizeof(nwmBeaconDataReplyEntry)],
                                      entry->size - sizeof(nwmBeaconDataReplyEntry), &networks_ptr[entpos]);
                if (R_FAILED(ret))
                    break;
...
Result udsBind(udsBindContext* bindcontext, u16 NetworkNodeID, bool spectator, u8 data_channel, u32 recv_buffer_size) {
    ...
    if (spectator) {
        pos = 0;
        if ((bind_allocbitmask & BIT(pos)))
            return -1;
    } else {
        for (pos = 1; pos < UDS_MAXNODES + 1; pos++) {
            if ((bind_allocbitmask & BIT(pos)) == 0)
                break;
        }
        if (pos == UDS_MAXNODES)
            return -1;
    ...
    if (spectator)
        bindcontext->BindNodeID |= spectator;
...
Result udsUnbind(udsBindContext* bindcontext) {
    ...
    if (!bindcontext->spectator)
        bitpos = bindcontext->BindNodeID >> 1;
...
bool udsWaitDataAvailable(const udsBindContext* bindcontext, bool nextEvent, bool wait) {
    ...
    if (nextEvent)
        svcClearEvent(bindcontext->event);

    if (svcWaitSynchronization(bindcontext->event, delayvalue) != 0 && !wait)
        ret = false;
...
static Result usd_parsebeacon(u8* buf, u32 size, udsNetworkScanInfo* networkscan) {
    ...
    {
        if (tagindex != 2 && (tags_data[tagindex] == NULL || tags_sizes[tagindex] == 0))
            return -3;

        if (tagindex && tags_sizes[tagindex] > 0xFE)
            return -3;
        if (tagindex == 1 && tags_sizes[tagindex] < 0x12)
            return -3;

        if (tagindex == 0 && ((tags_sizes[tagindex] < 0x34) || (tags_sizes[tagindex] > 0x34 + 0xC8)))
            return -3;
    }
    ...
    tagindex = 0;
    {
        ...
        if ((appdata_size > 0xC8) || (appdata_size > tag_datalen - 0x34))
            return -3; // Verify the appdata size.
        ...
        if (appdata_size)
            memcpy(networkscan->network.appdata, &tagptr[0x34], appdata_size);

        networkscan->network.initialized_flag = 1;
        networkscan->network.channel          = networkscan->datareply_entry.channel;
        memcpy(networkscan->network.host_macaddress, networkscan->datareply_entry.mac_address,
               sizeof(networkscan->network.host_macaddress));
    }

    memset(tmp_tagdata, 0, sizeof(tmp_tagdata));
    for (tagindex = 1; tagindex < 3; tagindex++) {
        if (tags_data[tagindex])
            memcpy(&tmp_tagdata[0xfe * (tagindex - 1)], tags_data[tagindex], tags_sizes[tagindex]);
    }
...
Result udsGetApplicationData(void* buf, size_t size, size_t* actual_size) {
    ...
    if (R_SUCCEEDED(ret)) {
        if (actual_size)
            *actual_size = cmdbuf[2];
    }

    return ret;
}

Result udsGetNetworkStructApplicationData(const udsNetworkStruct* network, void* buf, size_t size,
                                          size_t* actual_size) {
    if (network->appdata_size > sizeof(network->appdata))
        return -1;
    if (size > network->appdata_size)
        size = network->appdata_size;

    if (buf)
        memcpy(buf, network->appdata, size);
...
Result udsPullPacket(const udsBindContext* bindcontext, void* buf, size_t size, size_t* actual_size,
    ...
    if (R_SUCCEEDED(ret)) {
        if (actual_size)
            *actual_size = cmdbuf[2];
        if (src_NetworkNodeID)
            *src_NetworkNodeID = cmdbuf[3];
    }
