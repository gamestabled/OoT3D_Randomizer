...
static void Gfx_DrawSeedHash(void) {
    ...
    if (gSettingsContext.racingSetting == OFF || gSettingsContext.generateSpoilerLog == OFF || gSettingsContext.ingameSpoilers == ON) { return; }

    Draw_DrawString(10, 111, COLOR_TITLE, "Spoiler Log Passcode:");
    if (gExtSaveData.savedSpoilerLogPassCode == 0) {
        Draw_DrawString(10 + (SPACING_X * 4), 111 + SPACING_Y, COLOR_RED, "LOST! Don't copy or manipulate save files!");
    }
    else if (!SaveFileIsForThisSeed()) {
        Draw_DrawString(10 + (SPACING_X * 4), 111 + SPACING_Y, COLOR_RED, "This save file was not created for the current seed.");
    }
    else if (!gExtSaveData.reachedGanondorfWithItems) {
        Draw_DrawString(10 + (SPACING_X * 4), 111 + SPACING_Y, COLOR_WHITE, "????????");
    } else {
        Draw_DrawFormattedString(10 + (SPACING_X * 4), 111 + SPACING_Y, COLOR_WHITE, "%02u", gExtSaveData.savedSpoilerLogPassCode);
    }
}
