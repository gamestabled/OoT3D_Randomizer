...
void EnSyatekiMan_rInit(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((gSaveContext.linkAge == AGE_CHILD &&
         Entrance_SceneAndSpawnAre(
             0x42, 0x00)) || // Kakariko Village -> Adult Shooting Gallery, index 003B in the entrance table
        (gSaveContext.linkAge == AGE_ADULT &&
         Entrance_SceneAndSpawnAre(
             0x42, 0x01))) { // Market -> Child Shooting Gallery,           index 016D in the entrance table
        Actor_Kill(thisx);
    } else {
