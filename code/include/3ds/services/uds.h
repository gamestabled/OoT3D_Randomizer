 ...
 * Edited from
 * https://github.com/devkitPro/libctru/blob/c941b0d2d2409376fc80ed4da4cb1338ce8a71ed/libctru/include/3ds/services/uds.h
 */
#pragma once
...
/// Check whether a fatal udsSendTo error occured(some error(s) from udsSendTo() can be ignored, but the frame won't be
...
typedef struct {
    ...
    union {
        u8 usercfg[0x18]; // This is the first 0x18-bytes from this config block:
                          // https://www.3dbrew.org/wiki/Config_Savegame#0x000A0000_Block
...
typedef struct {
    ...
    u8 channel; // Wifi channel for this network. If you want to create a network on a specific channel instead of the
                // system selecting it, you can set this to a non-zero channel value.
    u8 pad_x7;

    u8 initialized_flag; // Must be non-zero otherwise NWM-module will use zeros internally instead of the actual field
                         // data, for most/all(?) of the fields in this struct.
...
enum {
    ...
    UDS_SENDFLAG_Broadcast = BIT(1)  // When set, broadcast the data frame via the destination MAC address even when
                                     // UDS_BROADCAST_NETWORKNODEID isn't used.
};
 ...
 * @param username Optional custom UTF-8 username(converted to UTF-16 internally) that other nodes on the UDS network
 * can use. If not set the username from system-config is used. Max len is 10 characters without NUL-terminator.
 */
 ...
 * @param username If set, this is the UTF-8 string to convert for use in the struct. Max len is 10 characters without
 * NUL-terminator.
 */
 ...
 * @param outbuf Buffer which will be used by the beacon-scanning command and for the data parsing afterwards. Normally
 * there's no need to use the contents of this buffer once this function returns.
 * @param maxsize Max size of the buffer.
 * @Param networks Ptr where the allocated udsNetworkScanInfo array buffer is written. The allocsize is
 * sizeof(udsNetworkScanInfo)*total_networks.
 ...
 */
Result udsScanBeacons(void* outbuf, size_t maxsize, udsNetworkScanInfo** networks, size_t* total_networks,
                      u32 wlancommID, u8 id8, const u8* host_macaddress, bool connected);
 ...
 * @param data_channel This is an arbitrary value to use for data-frame filtering. This bind will only receive data
 * frames which contain a matching data_channel value, which was specified by udsSendTo(). The data_channel must be
 * non-zero.
 * @param recv_buffer_size Size of the buffer under sharedmem used for temporarily storing received data-frames which
 * are then loaded by udsPullPacket(). The system requires this to be >=0x5F4. UDS_DEFAULT_RECVBUFSIZE can be used for
 * this.
 */
...
/**
 * @brief Waits for the bind event to occur, or checks if the event was signaled. This event is signaled every time new
 * data is available via udsPullPacket().
 ...
 * @param wait When true this will not return until the event is signaled. When false this checks if the event was
 * signaled without waiting for it.
 */
bool udsWaitDataAvailable(const udsBindContext* bindcontext, bool nextEvent, bool wait);

/**
 * @brief Receives data over the network. This data is loaded from the recv_buffer setup by udsBind(). When a node
 * disconnects, this will still return data from that node until there's no more frames from that node in the
 * recv_buffer.
 * @param bindcontext Bind context.
 * @param buf Output receive buffer.
 * @param size Size of the buffer.
 * @param actual_size If set, the actual size written into the output buffer is stored here. This is zero when no data
 * was received.
 * @param src_NetworkNodeID If set, the source NetworkNodeID is written here. This is zero when no data was received.
 */
Result udsPullPacket(const udsBindContext* bindcontext, void* buf, size_t size, size_t* actual_size,
 ...
 * @param context Optional output bind context which will be created for this host, with
 * NetworkNodeID=UDS_BROADCAST_NETWORKNODEID.
 ...
 */
Result udsCreateNetwork(const udsNetworkStruct* network, const void* passphrase, size_t passphrase_size,
                        udsBindContext* context, u8 data_channel, u32 recv_buffer_size);
 ...
 * @param recv_buffer_size This is the recv_buffer_size value which will be passed to udsBind() internally.
 */
Result udsConnectNetwork(const udsNetworkStruct* network, const void* passphrase, size_t passphrase_size,
                         udsBindContext* context, u16 recv_NetworkNodeID, udsConnectionType connection_type,
                         u8 data_channel, u32 recv_buffer_size);
...
/**
 * @brief This can be used by the host to force-disconnect the spectators. Afterwards new spectators will not be allowed
 * to connect until udsAllowSpectators() is used.
 */
Result udsEjectSpectator(void);

/**
 * @brief This can be used by the host to update the network attributes. If bitmask 0x4 is clear in the input bitmask,
 * this clears that bit in the value before actually writing the value into state. Normally you should use the below
 * wrapper functions.
...
/**
 * @brief This uses udsUpdateNetworkAttribute() for unblocking new spectator connections to this host. See
 * udsEjectSpectator() for blocking new spectators.
 */
...
/**
 * @brief Waits for the ConnectionStatus event to occur, or checks if the event was signaled. This event is signaled
 * when the data from udsGetConnectionStatus() was updated internally.
 * @return Always true. However if wait=false, this will return false if the event wasn't signaled.
 * @param nextEvent Whether to discard the current event and wait for the next event.
 * @param wait When true this will not return until the event is signaled. When false this checks if the event was
 * signaled without waiting for it.
