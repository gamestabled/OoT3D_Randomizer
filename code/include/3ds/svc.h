...
typedef enum {
    ...
    ARBITRATION_WAIT_IF_LESS_THAN =
        1, ///< If the memory at the address is strictly lower than #value, then wait for signal.
    ARBITRATION_DECREMENT_AND_WAIT_IF_LESS_THAN =
        2, ///< If the memory at the address is strictly lower than #value, then decrement it and wait for signal.
    ARBITRATION_WAIT_IF_LESS_THAN_TIMEOUT =
        3, ///< If the memory at the address is strictly lower than #value, then wait for signal or timeout.
    ARBITRATION_DECREMENT_AND_WAIT_IF_LESS_THAN_TIMEOUT =
        4, ///< If the memory at the address is strictly lower than #value, then decrement it and wait for signal or
           ///< timeout.
} ArbitrationType;
...
typedef enum {
    RESET_ONESHOT =
        0, ///< When the primitive is signaled, it will wake up exactly one thread and will clear itself automatically.
    RESET_STICKY =
        1, ///< When the primitive is signaled, it will wake up all threads and it won't clear itself automatically.
    RESET_PULSE = 2, ///< Only meaningful for timers: same as ONESHOT but it will periodically signal the timer instead
                     ///< of just once.
} ResetType;
...
typedef struct {
    s32 thread_ids[4]; ///< IDs of the attached process's threads that were running on each core at the time of the @ref
                       ///< svcBreakDebugProcess call, or -1 (only the first 2 values are meaningful on O3DS).
...
typedef struct {
    ...
    u32 flags; ///< Flags. Bit0 means that @ref svcContinueDebugEvent needs to be called for this event (except for EXIT
               ///< PROCESS events, where this flag is disregarded).
...
typedef enum {
    DBG_INHIBIT_USER_CPU_EXCEPTION_HANDLERS =
        BIT(0), ///< Inhibit user-defined CPU exception handlers (including watchpoints and breakpoints, regardless of
                ///< any @ref svcKernelSetState call).
 ...
 */
Result svcStartInterProcessDma(Handle* dma, Handle dstProcess, void* dst, Handle srcProcess, const void* src, u32 size,
                               void* dmaConfig);
 ...
 */
Result svcCreateThread(Handle* thread, ThreadFunc entrypoint, u32 arg, u32* stack_top, s32 thread_priority,
                       s32 processor_id);
 ...
 * @param nameCount Number of resource limit names.
 */
Result svcSetResourceLimitValues(Handle resourceLimit, const ResourceLimitType* names, const s64* values,
                                 s32 nameCount);
 ...
 *
 * This will perform an arbitration based on #type. The comparisons are done between #value and the value at the address
 * #addr.
 *
 * @code
 * s32 val=0;
...
/**
 * @brief Continues the current debug event of a debugged process (not necessarily the same as @ref
...
/**
 * @brief Fetches the saved registers of a thread, either inactive or awaiting @ref svcContinueDebugEvent, belonging to
 * a debugged process.
...
/**
 * @brief Updates the saved registers of a thread, either inactive or awaiting @ref svcContinueDebugEvent, belonging to
 * a debugged process.
 * @param debug Debug handle of the parent process.
 ...
 */
Result svcSetDebugThreadContext(Handle debug, u32 threadId, ThreadContext* context,
                                ThreadContextControlFlags controlFlags);
...
/**
 * @brief Sets an hardware breakpoint or watchpoint. This is an interface to the BRP/WRP registers, see
 * http://infocenter.arm.com/help/topic/com.arm.doc.ddi0360f/CEGEBGFC.html .
 * @param registerId range 0..5 = breakpoints (BRP0-5), 0x100..0x101 = watchpoints (WRP0-1). The previous stop point for
 * the register is disabled.
 * @param control Value of the control regiser.
 * @param value Value of the value register: either and address (if bit21 of control is clear) or the debug handle of a
 * process to fetch the context ID of.
 */
